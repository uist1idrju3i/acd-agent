...
- `scripts/placement_search.py`: 固定アンカー（RF モジュールを上端、USB コネクタを下端、
  ...
  primary と coupled member の全refdesを解決し、既配置メンバーからの宣言された最大距離を
  first-fit中に超える候補を除外する。`move_together`は現在、宣言された最大距離を使った
  first-fit制約であり、rigid co-transformではない。未知メンバー、重複group、壊れた距離、
  明示距離のない宣言はfail-closedで停止する。
