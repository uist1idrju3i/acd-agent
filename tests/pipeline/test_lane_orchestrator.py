...
def test_orchestrator_declares_firmware_after_barrier_and_before_controls(
    ...
    assert any("run_firmware_lane.py" in item for item in commands[3].command)
    assert commands[3].command[:3] == ("uv", "run", "python")
    assert "--script" not in commands[3].command
    assert "--with" not in commands[3].command
...
def test_lane_plan_preserves_gd1_paths_and_execution_contract(tmp_path: Path) -> None:
    ...
    assert plan.stage("projection-docs").output_path is None
    assert (
        plan.stage("manufacturing-submission").output_path
        == tmp_path / "manufacturing-submission.json"
    )
