...
def _minimal_plugin(tmp_path: Path) -> Path:
    ...
    hook_root = str(plugin_root / "hooks/scripts")
    ...
    - matcher: file_editor|apply_patch|terminal
      hooks:
        - type: command
          ...
          command: "python3 {hook_root}/protect_projections.py"
    - matcher: terminal
      hooks:
        - type: command
  ...
  stop:
    - hooks:
        - type: command
          ...
          command: "python3 {hook_root}/stop_policy.py"
---
Test agent.
""",
        encoding="utf-8",
                        ...
                        "hooks": [
                            {
                                ...
                                "name": "protect-derived-projections",
                                "command": (
                                    f"python3 {hook_root}/protect_projections.py"
                                ),
                            }
                        ],
                    },
                        ...
                        "hooks": [
                            {
                                "type": "command",
                                "name": "require-gate-after-input-change",
                                "command": f"python3 {hook_root}/stop_policy.py",
                            }
                        ]
                    }
                ],
...
def test_bootstrap_wires_sdk_conversation_without_llm_call(tmp_path: Path) -> None:
    ...
    assert len(conversation.agent.agent_context.skills) == 16
