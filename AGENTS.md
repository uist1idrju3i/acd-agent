...
> 対象: OpenHands Software Agent SDK v1.47.0、Python 3.12+
...
├── hooks/
└── .plugin/plugin.json
vendor/software-agent-sdk/       # OpenHands SDK v1.47.0のみ
```

本リポジトリはOpenHands Software Agent SDK v1.47.0専用拡張であり、機能採否は
...
- 外部ネイティブ拡張（OCP等）でfork継承が不安全な場合は、spawn workerをpipeline内で再利用する。
  worker起動コストが支配的な規模では既定を逐次にし、並列は明示指定のopt-inとする。
- CAD以外の独立laneも、共有fixtureの更新をbarrierで完了させてから並列化し、共有書き込み先が
  ある場合は分離または逐次化を確認してから実行する。
...
依存の更新確認は[`.github/workflows/check-dependency-updates.yml`](.github/workflows/check-dependency-updates.yml)（週次 + 手動）が`scripts/check_dependency_updates.py`で行い、更新候補をIssue「依存アップデート確認レポート」へ報告する。確認対象はPyPI直接依存・`uv.lock`間接依存、`vendor/software-agent-sdk`のsubmoduleと3つのSDK pin、GitHub Actionsとrelease download、Docker base imageとARG、image toolsの上流版、Python版、`libraries/README.md`のgit pin、Semeruの新majorである。PEP 723のgit ref、versionを記録しないaptパッケージ、ESP-IDF toolchain binary、vendor内agent-server base imageは別管理のため確認しない。**依存を追加・削除・移動した場合、Dockerfileへ新しいバージョンARGを追加した場合、新しい外部取得元（PyPI以外、別のGitリポジトリ、apt/PPA等）を使い始めた場合は、同じ変更で`scripts/check_dependency_updates.py`の対象定義（`DOCKER_ARG_SPECS`、`TOOL_UPSTREAM_SPECS`など）とテスト、`docs/operations.md`の該当節を更新し、`uv run python scripts/check_dependency_updates.py`をローカルで実行して対象が漏れていないことを確認する。新しい外部ツールをimageへ追加し`docker/image-digests.json`の`tools`へ記録する場合は`TOOL_UPSTREAM_SPECS`へも上流取得元を追加する。** 自動更新PRは作らず、更新は本節の手順に従って人手／Devinが行う。
