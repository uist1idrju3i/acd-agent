...
| 候補 | ライセンス | 判定 | 根拠 |
|---|---|---|---|
| [KiKit](https://github.com/yaqwsx/KiKit) | 本文・現行版を一次確認前 | 継続調査 | 単体基板から面付け投影を生成する候補。CLI契約、KiCad版互換、ヘッドレス実行可否をPhase 0で確認する |
| ACD側の幾何生成adapter | ACD実装 | 継続調査 | 外形、タブ、切り分けを構造化パラメータから生成する代替候補だが、採否は未決 |

面付けは単体基板から面付け投影を生成する工程として扱い、`S2`製造出力は面付け投影から
生成する。行列、基板間隔、連結タブ、Vカット／ミシン目、レール、位置決め穴、フィデューシャル、
工具半径などを構造化して保存する。面付け後はDRCと独立再読込を実行し、面付け前のEvidenceを
流用しない。
...
- 外部プロセスのbinaryはACD配布物へ同梱しない前提で設計し、同梱が必要になった時点で
  法務確認を行う。
- 座標の単位、原点、軸、面、回転基準はadapter入力で固定し、既定値に依存しない。
- ゾーン等の派生状態を再計算する方法と、再計算結果を独立に確認する方法をPhase 0で確認する。
- 外部ライブラリはcommitでpinし、ライブラリテーブルとパス変数の解決結果を実パス・版付きで記録する。
- pluginと処理backendが分離する候補は、互換性と版を個別に固定する。設定ディレクトリは隔離する。
- GPU・表示環境に依存しない経路を既定とし、描画経路はrendererと代替経路を記録する。
- 視覚レビュー用rendererは、画像hash、解像度、renderer版、表示依存をEvidenceへ記録できる
  ものを選ぶ。視覚投影は`ImageContent`または`inspect_image_with_vision`へ渡せるが、
  描画結果を決定論的ゲートの入力にはしない。
- ファイル形式版の非可逆更新が起きるかを能力プローブで確認し、ツール版と形式版を併せてpinする。
...
- 面付けツールの採否、variant／DNP出力のCLI対応、ライブラリテーブル解決結果。
- ルール重大度・除外の機械的扱い、検査レポートの機械可読形式、形式版更新挙動。
