...
GD1最新実行で生成される製造データは、`out/gd1-fix4/fab/`の次のファイルである。
...
SMD pad上viaを構造的に禁止している。出所は`out/gd1-fix4/fab/dfm-report.json`、
`out/gd1-fix4/routing-summary.json`、`out/gd1-fix4/fab/fab-package.json`である。
