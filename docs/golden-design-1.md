...
インセットで定義する。KiCad zoneのfillは`island_removal_mode 0`で孤立島を常に除去し、
`ground_plane_min_island_area_mm2`は塗り後Gerber検証でConductor領域の最小面積として検査する
（KiCadが`island_removal_mode 2`で保持する島は`NonConductor`として出力され、GND接続点を
持たない浮き銅となるため採用しない）。塗りは自前計算せず、KiCad 10.0.5の`--refill-zones --save-board`
...
筐体材質は発注可能なMJF **PA12-HP nylon**とし、締結方式は**M2セルフタッピングねじ**
（`self_tapping_screw_m2`）に固定する。各スタンドオフにはφ1.6 mmのパイロット穴を
スタンドオフ高さ分だけ設け、シェル底面は貫通させない。リッドには
`outline.mount_holes`と同じ座標でφ2.2 mmのクリアランス穴を各1個設ける。基板端から
突出するU1アンテナは`mechanical.board_edge_overhang`の宣言を消費してシェルの切欠きへ
反映し、干渉ゲートでも同じ突出領域を3D固体として検査する。
...
総発注額は、基板、部品、実装、送料、税を含める。[`roadmap-completed.md`](roadmap-completed.md)の
