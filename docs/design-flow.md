...
> ステータス: Draft  
> 対象: Phase 1〜3の基板・筐体・レーン統合フロー、OpenHands SDK v1.41.0
...
6段階の各工程には、定義済みの入力、出力、自動検証ゲート、知識取得フックがある。
...
- **自動検証ゲート:** 単位、矛盾、未回答の必須項目、電気・機械の境界条件、総発注額の
  ...
  抽出し、安全境界を予備判定する。禁止領域なら設計を始めない。曖昧な項目は仮定にせず
  質問または`unknown`にする。予備判定は`SafetyBoundaryResult`として記録する。
...
- **自動検証ゲート:** pin/data-sheet整合、ERC、電源予算、部品入手性、必要なSPICE解析に加え、
  ...
  `Part.certification`）で安全境界を確定判定する。S2の判定をゲートの正とし、禁止領域、
...
- **診断と還流:** 測定結果を記録する。問題発生時は設計根拠を手がかりに候補根本原因まで
  遡って診断する。設計改訂は完全な変更追跡とともにS2〜S4、機械レーン、FWレーンへ戻す。
