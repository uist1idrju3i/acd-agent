...
各工程の出口では機械可読投影と視覚投影を生成してAIレビューを行い、決定論的ゲートで判定する。
所見は自然文で次の修正へ渡す。
...
詳細な投影セットとレビュー観点は[`projection-review.md`](projection-review.md)を正とする。
...
- **知識フック:** 過去の確認質問、要求パターン、fab・筐体の失敗モードをQ7/N7の作業手法で
  レビュー観点として参照する。
...
- **筐体側:** CAD kernelでvalidity・干渉を再計算し、再計算前の3D modelを合格根拠にしない。
- **出口投影とAIレビュー:** 断面・干渉ビュー、組立順序図、寸法投影をレビューし、
  詳細は[`projection-review.md`](projection-review.md)で定める。

## S2 製造出力

- **入力:** 共通ゲート合格の入力ファイル、fab profile、部品・材料Evidence。
...
- **自動検証ゲート:** profileが宣言するexport formatで出力し、独立parserで再読込してhash、
  ...
  発注前最終ゲートでは、設定上限額と全ゲートの通過を検査し、いずれかが不明なら発注しない。
...
- **DFM所見:** 次の所見を扱う。
  ...
  - インピーダンスのstackup不一致
  - 実装時の部品入手性
- **出力:** 構造化所見、改訂案、次の修正へ渡す作業手法。
- **自動検証ゲート:** 指摘の出所、git commit、再現条件、測定適格性を確認する。
...
- **知識フック:** 失敗モード、測定マージン、根本原因、成功条件をSkill・pluginの作業手法へ反映する。
...
AIは勝手に優先度を変更しない。矛盾や未知の影響は合格扱いせず、変更した入力ファイルを
起点に全ゲートを再実行し、決定を根拠として記録する。

## ファームウェア・部品ライブラリとの往復

ファームウェア側からピン割当の変更要望が出た場合は、設計根拠と変更箇所を付けて
入力ファイルのE1の機能ブロック、部品、ネットの選定へ戻す。変更後はE2の配置・配線、S2の
...
作業手法へ反映し、同じ指摘を次の設計で繰り返さないための知識とする。公式ライブラリ
自体は変更せず、修正は`LibraryOverlay`としてプロジェクトローカルに保持する。

## 電気とファームウェアの調停

ピン割当を変更するか、回路・部品・ネットを変更するかは、要件の優先度、コスト、
性能、実装影響、検証Evidenceに基づいて比較する。選択した調停結果と、入力ファイルへ
反映した変更範囲を設計根拠として記録する。
