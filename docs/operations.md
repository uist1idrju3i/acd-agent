...
確認手順と対象の記録は[`dependency-updates.md`](dependency-updates.md#依存アップデートの確認)へ移した。
...
OrthoRouteと他の代替routerの単独実測と不採用の記録は[`measurement-records.md`](measurement-records.md)へ移した。
...
### core／Skill間のimport境界

`scripts/verify_import_boundaries.py`（fast段階）は`ast`でimportを走査し、
(1) `src/acd`・`scripts/`がSkill／hooks scriptのmoduleまたは`plugins`をimportする、
(2) `src/acd`が`sys.path`を変更する、(3) Skill scriptが別Skillの`scripts/`をimportする、
のいずれかをfail-closedで検出する。Skill scriptから`acd.*`へのimportはpackage contract
（`verify_skill_package_ref.py`）で管理され、この検査の対象外である。
Skillのtestは`plugins/acd/skills/conftest.py`の`pytest_pycollect_makemodule`で
自Skillの`scripts/`が`sys.path`へ追加されるため、test本文や各Skillの`conftest.py`で
`sys.path.insert`を書かない。`scripts/`直下の入口はすべて
[`scripts/README.md`](../scripts/README.md)で分類し、分類漏れは
`scripts/tests/test_scripts_inventory.py`が検出する。
...
更新時の記録先は[`dependency-updates.md`](dependency-updates.md#依存版破壊的変更の記録)である。
...
実行記録は[`measurement-records.md`](measurement-records.md#非gd1-fixturemini-blink-dongleの実行記録)へ移した。
