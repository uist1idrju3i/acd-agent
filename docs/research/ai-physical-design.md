...
LLMは**探索空間と評価方針の宣言**を担い、幾何の整合化と合否判定は決定論器が担う。

| 層 | 担当 | 出力 | LLMの関与 |
|---|---|---|---|
| L1 宣言層 | LLM（設計agent） | 探索仕様（モジュール分解、相対配置制約、優先度、探索・評価方針） | あり。具体的な座標案を含めてもよい |
...
- 幾何の可否は数値精度と実測に依存するため、もっともらしい出力はどちらにしても合格根拠にならない。
  PCB向けのマルチモーダルbenchmarkでは、配置・制約推論の正答率は実務水準に届いていない
  （[`prior-art.md`](prior-art.md)の§21）。
- 反復コスト（token、money、wall-clock）が候補数に比例し、探索の予算と打ち切りを事前に宣言しにくい。
- 失敗したときに、原因をどの宣言へ帰属させればよいかが決まらず、修復の往復が収束しにくい。

要求するのは「毎回同じ設計が出ること」ではない。実行ごとに異なる解になってよく、探索としては
むしろ多様な候補が得られる方が望ましい。要求するのは、出た設計を後から再検証できること、
すなわち各候補の入力、seed、ツール版、入力hashを再確認できることだけである。
この条件を満たさない候補は決定論的ゲートへ送らない。
...
flowchart LR
    ...
    L3 --> PR["機械可読投影＋視覚投影"]
    PR --> RVW["別コンテキストAIレビュー<br/>自然文の所見"]
    RVW --> GATE["決定論的ゲート"]
    GATE -->|fail| L1
    L3 -->|ゲート不合格| L1
```
...
- 視覚投影（配置図、層別レイアウトビュー、Gerber render）は問題の**発見**に使う。
  ...
  （[`prior-art.md`](prior-art.md)の§21）。
- レビュー結果は自然文の所見とし、投影上で観察した根拠を引用させる。所見は修正ループへ渡し、
  合格根拠にはしない。
- 実測、ERC/DRC、Gerber再読込、発注前最終ゲートは決定論的ゲートへ集約する。
...
- 同じ所見が繰り返される場合は、探索ではなく要件・部品・外形の問題としてエスカレーションする。
...
- LLMの座標・回転角の提案を、投影と決定論的ゲートを通さずに合格として扱う。
