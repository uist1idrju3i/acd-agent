...
| sdk.conversation.secrets_manager | `SecretRegistry` | secret注入・漏洩防止 | 採用 | `LocalConversation(secrets=...)`経由でSDK内部registryへallowlist sourceを渡し、maskingはSDK内部（terminal tool）で行う | registry回帰 |
...
| sdk.secret | `SecretSource`<br>`StaticSecret`<br>`LookupSecret` | secretを平文から分離 | 採用 | allowlist環境変数を`SecretSource`として`LocalConversation(secrets=...)`経由でSDK内部registryへ渡し、`StaticSecret`/`LookupSecret`は直接importしない | registry masking回帰 |
...
| tools.delegate | `DelegateExecutor` | sub-agent調整 | 採用 | SDKのTask/delegate実行経路を採用するが、ACD側から`DelegateExecutor`を直接参照しない。hookと確認方針を境界にする | session回帰 |
