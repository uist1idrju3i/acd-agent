...
| sdk.profiles | `AgentProfile`, `AgentProfileStore` | secret-free profile配布 | 採用（文書上の方針のみ、実装未着手） | profile driftを管理し、資材hashを固定する | pinned API確認、実装未着手 |
...
| sdk.context.memory | `MEMORY.md` memory | 作業文脈の補助 | 採用（文書上の方針のみ、実装未着手） | 契約・合否の正にしない。資材hashを固定する | pinned API確認、実装未着手 |
| sdk.context.prompts | `Prompt`, `PromptSection`, preset/registry/section、static/dynamic/planning | role別prompt構造化 | 採用（文書上の方針のみ、実装未着手） | role別promptと資材hashを固定する | pinned API確認、実装未着手 |
| sdk.context.view | context/event view properties | 長時間sessionの表示 | 採用（文書上の方針のみ、実装未着手） | 原EventLogと照合し、Evidenceを置換しない | pinned API確認、実装未着手 |
...
| sdk.tools.workflow | `WorkflowToolSet`, `map_agents`, `reduce_agent`, `pipeline` | lane並列化のmap/reduce | 不採用（将来再検討） | LLMが書いた任意Python scriptがhook matcherの外側で実行され、fail-closed境界を貫通しうる | pinned API確認、採用しない |
...
| sdk.llm.router | `RouterLLM`, `RandomRouter`, `MultimodalRouter` | judge/critic用modelと主agent用modelの分離 | 採用（文書上の方針のみ、実装未着手） | routing結果は合否へ影響させず、profileと資材hashを固定する | pinned API確認、実装未着手 |
| sdk.observability | `observe`, Laminar初期化 | L3観測の構造化 | 採用（文書上の方針のみ、実装未着手） | 送信先・sanitizer・既定無効を固定し、Evidenceへ触れない | pinned API確認、実装未着手 |
| sdk.io | `FileStore`, `LocalFileStore`, `InMemoryFileStore` | session保存抽象 | 採用（文書上の方針のみ、実装未着手） | git/inputを正とし二重の合否状態を持たない | pinned API確認、実装未着手 |
| sdk.logger | structured logger | L3観測の構造化 | 採用（文書上の方針のみ、実装未着手） | secretとEvidenceをログへ混入させない | pinned API確認、実装未着手 |
| sdk.settings | `settings` | SDK設定の外部化 | 採用（文書上の方針のみ、実装未着手） | 設定資材のhashを記録し、unknownはfail-closedにする | pinned API確認、実装未着手 |
| sdk.credential | `credential.py` | 資格情報の外部化 | 採用（文書上の方針のみ、実装未着手） | secret allowlistを維持し、unknownはfail-closedにする | pinned API確認、実装未着手 |
...
| tools.browser_use | browser toolset | L2の部品調査・datasheet確認 | 採用（L2探索のみ、Evidence昇格禁止） | 決定論的APIがない場合の探索補助。設計入力は既存APIで再取得しhash固定 | `BrowserToolSet.is_usable()`、agent session回帰 |
...
| workspace.LocalWorkspace | `LocalWorkspace` | agent作業workspace | 採用（文書上の方針のみ、実装未着手） | host実行はprovisionalに限定し、authoritative経路にしない | pinned API確認、実装未着手 |
| workspace.DockerWorkspace | `DockerWorkspace` | 事前build済みdigest固定server imageの実行 | 採用（文書上の方針のみ、実装未着手） | #65のauthoritative経路はDockerDevWorkspace。配布済みserver image移行後に採用 | constructor確認、実装未着手 |
...
| agent-server.conversation_router | conversation endpoints | server対話 | 保留（着手判断未決） | agent-serverを採用する設計判断とAPI受入試験が完了した場合のみ着手 | 未検証 |
| agent-server.event_router | event endpoints | server event取得 | 保留（着手判断未決） | L3観測に限定しEvidence化しない | 未検証 |
| agent-server.tool_router | `tool_router` | server tool routing | 保留（着手判断未決） | 現行agent入口ではなく将来構想 | 未検証 |
| agent-server.tool_preload_service | `tool_preload_service` | server tool preload | 不採用 | 現行実行形に直接依存しない | 未検証 |
| agent-server.settings_router | `settings_router` | server設定API | 保留（着手判断未決） | server着手時の設定境界 | 未検証 |
| agent-server.llm_router | `llm_router` | server LLM routing | 不採用 | 暗黙routingはmodel固定と非両立 | 未検証 |
| agent-server.init_router | `init_router` | server初期化 | 保留（着手判断未決） | 将来server起動契約の一部 | 未検証 |
| agent-server.server_details_router | `server_details_router` | server情報 | 保留（着手判断未決） | 将来運用診断の一部 | 未検証 |
| agent-server.workspace_router/workspaces_router | workspace routers | server workspace管理 | 保留（着手判断未決） | DockerWorkspace受入時に検証 | 未検証 |
| agent-server.profiles_router/agent_profiles_router | profile routers | profile配布 | 保留（着手判断未決） | secret-free profileを条件にする | 未検証 |
| agent-server.credential_binding | `credential_binding` | server credential binding | 保留（着手判断未決） | SDK secret経路と分離して検証 | 未検証 |
| agent-server.middleware | middleware | server認証・境界 | 保留（着手判断未決） | 直接APIへの適用範囲を確認する | 未検証 |
| agent-server.pub_sub | `pub_sub` | server event配信 | 不採用 | SDK Conversation/eventを正とし直接依存しない | 未検証 |
| agent-server.openapi | OpenAPI schemas | server API投影 | 保留（着手判断未決） | API利用時のみ契約投影として使う | 未検証 |
...
| agent-server._secret_redaction/_secrets_exposure | secret redaction | server secret漏洩防止 | 保留（着手判断未決） | server採用時の停止境界に限定 | 未検証 |
...
| agent-server.sockets | socket APIs | server transport | 保留（着手判断未決） | server着手時の通信境界 | 未検証 |
| agent-server.telemetry | telemetry exporters | server L3観測 | 保留（着手判断未決） | SDK observabilityと混同せずserver時のみ | 未検証 |
| agent-server.persistence | persistence APIs | server Conversation保存 | 保留（着手判断未決） | SDK conversation保存と別のserver機能 | 未検証 |
| agent-server.conversation_lease | `conversation_lease` | server所有権 | 保留（着手判断未決） | 複数instance受入時に検証 | 未検証 |
| agent-server.docker | server Docker build | server image build | 保留（着手判断未決） | digest記録を条件にする | 未検証 |
