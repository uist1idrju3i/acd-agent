...
> 対象: v1.49.2 / submodule commit `d128a786ee2ee570eb23ff5862ec148b43cfad0b`
>
...
| sdk.llm.cleanup | `clean_outward_text`<br>`aclean_outward_text` | 出力文面の補正 | 不採用 | cleanup LLMはfail-openの補助surfaceであり、ACDのauthoritative Evidenceや合否へ介入させない | pinned API確認、採用しない |
| sdk.llm.internal | `LLMProfileStore` | SDK内部provider補助 | 不採用 | provider詳細・内部補助をACDが直接依存しない | pinned API確認、採用しない |
| sdk.llm.provider_connections | `ProviderConnection`<br>`ProviderConnectionStore` | LLM接続情報の共有保存 | 不採用 | provider connectionはSDKの補助surfaceであり、ACDの認証情報管理契約を変更しない | pinned API確認、採用しない |
...
| sdk.llm.runtime_metadata | `ModelRuntimeMetadata`<br>`resolve_provider_metadata_sync` | provider routeの実行時メタデータ | 不採用 | runtime metadataはrouting補助であり、ACDのauthoritative Evidence、閾値、合否を変更しない | pinned API確認、採用しない |
...
| tools.ask_oracle | `AskOracleTool` | 別LLM profileへの助言照会 | 不採用 | `oracle` profileの応答は非決定論的なL2助言であり、tom_consultと同じく合否へ関与しない経路を増やさない | pinned API確認、採用しない |
...
| tools.preset.task_outcome | `TaskOutcome` | structured task outcome | 不採用 | experimentalなstructured outcomeはACDの決定論的L1判定を置き換えない | pinned API確認、採用しない |
...
| workspace.agent_sandbox | `AgentSandboxWorkspace` | Kubernetes agent-sandbox実行 | 不採用 | `DockerWorkspace`以外の実行環境はOpenHands専用拡張の境界外 | pinned API確認、採用しない |
