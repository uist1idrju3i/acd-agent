...
これら2種の視覚投影をL3観測として扱う。8.3のゲート通過後の既定生成、8.4のAI受け渡し、
8.5の機械可読投影との照合は未実装であり、[`roadmap.md`](roadmap.md)に従って後続で扱う。
...
画像hash、renderer種別、解像度の記録schemaは8.1で実装済みである。KiCad SVGの
