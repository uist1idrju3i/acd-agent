...
利用者のOpenHands環境へACDを導入する手順が重かった。pinned SDK v1.42.1は拡張の
標準配布機構として、`PluginSource`／`Plugin.fetch()`によるgit取得（resolved commit
SHAへのピン留めと`~/.openhands/cache/plugins/`へのキャッシュ）を提供する。また
`openhands-sdk`、`openhands-tools`、`openhands-workspace`のv1.42.1はPyPIへ公開済み
...
- `pyproject.toml`の`[project.dependencies]`で`openhands-sdk==1.42.1`、
  `openhands-tools==1.42.1`、`openhands-workspace==1.42.1`をPyPI版へピン留めする。
...
- SDK submodule版とPyPI pinの版は同一（v1.42.1）でなければならない。submodule更新時は
