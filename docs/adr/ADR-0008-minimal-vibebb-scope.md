...
> 追補: 視覚投影の`ImageContent`／`inspect_image_with_vision`経路はマイルストーン8.4で実装済みであり、画像受け渡しはworkspace内PNGの`data:` URLに限定する。
...
高信頼化は[`../roadmap-future.md`](../roadmap-future.md)の将来展望としてのみ扱い、規範として先取りしない。
