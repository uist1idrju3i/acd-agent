...
レビュー投影セットは影響分析で選択し、必要な対象だけを生成する。投影は、AIが表や
netlist要約を読むための**機械可読投影**と、図・3Dビュー・レイアウトビューを観察する
ための**視覚投影**に分類する。視覚投影はレビューの観察入力であり、決定論的ゲートの
入力にはしない。

| 分類 | 投影 | AIへ渡す経路 |
|---|---|---|
| 機械可読 | netlist要約、ピン割当、要求トレーサビリティマトリクス、変更影響マトリクス | 構造化データまたはテキスト |
| 視覚 | ブロック図、回路図、電源ツリー、配置図、層別レイアウトビュー、stackup図 | `ImageContent`または`inspect_image_with_vision` |
| 視覚 | 外形・断面・干渉ビュー、組立順序図、寸法投影、Q7/N7図表 | `ImageContent`または`inspect_image_with_vision` |
| 機械可読 | 電源ツリー、信号経路、FW設定表、メモリマップ、Q7/N7の元データ | 構造化データまたはテキスト |
