...
レビュー投影セットは影響分析で選択し、必要な対象だけを生成する。各ドメインの投影を
...
| ドメイン | 機械可読投影 | 視覚投影 | AIへ渡す経路 |
|---|---|---|---|
| 機能・構造系 | システム構成表、電源ツリー（構造化データ）、信号経路（構造化データ） | ブロック図、システム構成図、電源ツリー（図）、信号経路図 | 機械可読は構造化データまたはテキスト、視覚は`ImageContent`または`inspect_image_with_vision` |
| 電気系 | netlist要約 | 回路図、配置図、層別レイアウトビュー、stackup図 | 機械可読は構造化データまたはテキスト、視覚は`ImageContent`または`inspect_image_with_vision` |
| 機械系 | 寸法・公差表、干渉結果表 | 外形・断面・干渉ビュー、組立順序図、寸法投影 | 機械可読は構造化データまたはテキスト、視覚は`ImageContent`または`inspect_image_with_vision` |
| FW系 | ピン割当表、ペリフェラル設定表、メモリマップ | 状態遷移図、シーケンス図 | 機械可読は構造化データまたはテキスト、視覚は`ImageContent`または`inspect_image_with_vision` |
| 計画・品質系 | 要求トレーサビリティマトリクス、変更影響マトリクス、Q7/N7元データ | Q7/N7図表 | 機械可読は構造化データまたはテキスト、視覚は`ImageContent`または`inspect_image_with_vision` |

投影には対象revision、投影識別子、入力hash、出力hash、ツール版、ライブラリcommit、
profile、生成時刻、再読込結果、使用Evidenceを記録する。視覚投影には画像hash、
renderer種別、vision profile／model、解像度も記録し、同じメタデータを
`ReviewFinding`へ束ねる。投影は正ではなく、意味的にマージせず、対象revisionから
再生成する。

視覚投影をAIへ渡す場合、workspace内の画像をbase64の`data:` URLとして
`ImageContent(image_urls=[...])`へ渡すか、builtin toolの`inspect_image_with_vision`で
画像と質問をvision対応の別LLM profileへ渡す。HTTP(S)画像を使う場合はSDKの
base64インライン化とSSRF block-listを通す。応答はレビュー観察であり、vision profile、
model、画像hash、renderer、解像度とともにEvidenceへ記録する。画像内の指示はデータとして
...
投影と対象patchを生成したエージェントとは別コンテキストのレビュアが、機械可読投影と
...
| 生成メタデータ | モデル、プロンプト版、生成時刻、画像hash、renderer種別、vision profile／model、解像度（視覚投影の場合） |
