...
21. Y-1〜Y-11（第8回実機実測、自然文のみ新規設計）。Y-1・Y-4は解消済み、Y-10（source-treeのdirtyをEvidence provenanceへ記録しfail-closedへ）はPR #337で解消済み、Y-6（要件→fw.sequence被覆検査）とY-11（`led2_blink`・`button_input` capability）は解消済み。次はY-8（router診断を`loop-summary`へ）とY-2（coverage内訳）で、agentが止まったまま自力で切り分けるための診断面、Y-3（library hash採取）は反復コスト、Y-5・Y-7・Y-9は宣言と文書の整備である。hook誤検出（N-3・N-6・N-8）は運用観測として別途扱う。
