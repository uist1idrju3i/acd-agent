...
Y-1とY-4は本変更で解消した。Y-6の被覆検査（`check_firmware_coverage`と
`emits_triggers`）も解消したが、複数LED・入力capability本体はY-11に残る。
Y-10はEvidence provenance面の追加であり、合格側権限の
...
21. Y-1〜Y-11（第8回実機実測、自然文のみ新規設計）。Y-1・Y-4は本変更で解消済み、Y-10（source-treeのdirtyをEvidence provenanceへ記録しfail-closedへ）はPR #337、Y-6（要件→fw.sequence被覆検査）は本変更で解消済み。残るうちY-11（FW capability契約の複数LED・入力対応）はfail-closed境界の堅持に直結し先に扱う。Y-8（router診断を`loop-summary`へ）とY-2（coverage内訳）はagentが止まったまま自力で切り分けるための診断面、Y-3（library hash採取）は反復コスト、Y-5・Y-7・Y-9は宣言と文書の整備である。hook誤検出（N-3・N-6・N-8）は運用観測として別途扱う。
