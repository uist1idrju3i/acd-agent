...
Y-1・Y-4・Y-10・Y-8はそれぞれの変更で解消した。Y-10はEvidence provenance面の追加、
Y-8は`loop-summary`へのL3診断追加であり、いずれも合格側権限の緩和ではない。N-3・N-6・N-8（hookの誤検出）は運用観測として`notes.md`に留め、
優先順位の末尾に記録する。
...
21. Y-1〜Y-11（第8回実機実測、自然文のみ新規設計）。Y-1・Y-4・Y-10・Y-8は解消済み。残るうちY-11／Y-6（FW capability契約と要件→fw.sequence被覆検査）はfail-closed境界の堅持に直結し先に扱う。Y-2（coverage内訳）はagentが止まったまま自力で切り分けるための診断面、Y-3（library hash採取）は反復コスト、Y-5・Y-7・Y-9は宣言と文書の整備である。router壁そのもの（筐体face契約を含む後続判断）はY-8の診断が揃った後に扱う。hook誤検出（N-3・N-6・N-8）は運用観測として別途扱う。
