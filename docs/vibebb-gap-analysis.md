...
O-1・O-9は「設計内容に依らず基板lane以降へ到達できない」直接原因、O-10はFW laneがGD1専用実装である直接原因、O-12は筐体laneと発注可否判定がGD1専用である直接原因、O-11はhook契約の矛盾、O-13は診断語彙の誤導、O-2は実行基盤側の律速であり、O-4・O-5は
...
| O-2 | container起動がホスト資源を検査しない | `src/acd/openhands/container_runtime.py`の`DEFAULT_MEMORY_LIMIT`は`"8g"`で、`--memory=8g --memory-swap=8g`を付けて`docker run`する。ホスト実測値（メモリ実装1641 MiB、swap 5116 MiB、CPU 3コア）との整合は検査しておらず、要求上限8 GiBはメモリ＋swap合計（約6.6 GiB）を超える。実機ではcontainer gate実行中に`A restart occurred while this tool was in progress. This may indicate a fatal memory error or system crash.`が4回以上発生し、sshd不応答を経てOpenHandsサーバプロセスが停止した。ホスト`dmesg`には`constraint=CONSTRAINT_NONE ... global_oom`と、`docker-<container>.scope`内の`java`（FreeRouting、total-vm約5.2 GiB）2プロセスのkillが記録されており、`dbus-daemon`・`systemd`・`node`・`python`まで巻き込まれている。JVM最大ヒープを明示していないため、JVMはcontainer上限の約1/4を最大ヒープとして採る | 高 | なし | container起動前にホストの`MemTotal`／`MemAvailable`／swap／ディスク空きと要求上限を比較し、超過時は起動せず理由付きでfail-closedにする。JVMを使う外部ツールの最大ヒープを明示的に宣言・制御し、実行条件としてprovenanceへ記録する。`/acd:doctor`へ同項目を追加する。最小ホスト要件を`docs/`へ記録する |
...
| O-10 | FW laneがGD1のnet集合とGD1のapplication codeを定数で持ち、他設計のfirmwareを生成できない | `plugins/acd/skills/acd-firmware-esp32c3/scripts/fw_project.py`は`_REQUIRED_NETS`として`net.led`／`net.i2c_sda`／`net.i2c_scl`／`net.uart_tx`／`net.uart_rx`／`net.boot`／`net.usb_dn`／`net.usb_dp`をmodule定数で持ち、`render_pins_header()`はこの全netのGPIOを要求する。さらに生成される`main.c`はGD1固定の内容（`SHT40_I2C_ADDRESS = 0x44`、`i2c_master_*`によるSHT40読み出し、1 Hz点滅ログ）をtemplateとして埋め込む。実機ではI2C・UARTを持たない`pulse-check-tag`に対し`PIPELINE FAILED: no firmware pin assignment for net 'net.i2c_sda'`でfail-closedし、`[1/5] firmware project projected`へ到達しなかった。センサを持たない設計にI2C netを宣言させれば通過しうるが、それは設計意図の改変であり回避策として採らない | 高 | J-2 | 必要netと生成codeをlane固定の定数ではなく設計graph（機能ブロック宣言とpin function）から導出する。net集合をfixtureまたは契約registryの宣言から取得し、宣言に無いperipheralのmacro・初期化・読み出しcodeを生成しない。application挙動（点滅周期、入力によるtoggle等）を要件宣言から受ける。GD1の生成物・判定・正規化hashは不変に保ち、GD1以外でも`[5/5]`まで到達できることを回帰テストで固定する |
| O-11 | projection guardがlaneの出力先指定とstop reportの記録をdenyし、引用を変えると通る | `plugins/acd/hooks/scripts/protect_projections.py`は`PROTECTED = ("out", "evidence")`を持ち、`terminal` toolのコマンド文字列を`shlex.split`して`out/`配下を指すトークンを検出すると、先頭コマンドが`READ_COMMANDS`以外ならdenyする。実機ではFW lane再実行が`rejection_source: hook`でdenyされた。同時に`stop_policy.py`の`STOP_REPORT_PATH = "out/stop-report.json"`への記録も同じ規則でdenyされ、fail-closed状態を宣言して停止する規定経路が塞がれる。またhook scriptへ直接payloadを与えた検査（host実行、参考値）では`uv run --script x.py --out out/fw`はdeny、`bash -c "uv run --script x.py --out out/fw"`と`python3 -c "...open(\"out/stop-report.json\",\"w\")..."`はallowとなり、内側を1トークンへ包むだけで迂回できる | 高 | なし | 保護対象を生成物ファイルへの書き込み・編集に限定し、laneの出力先指定引数（`--out`／`--download`）と`out/stop-report.json`への記録を明示的に許可する。判定をtoken一致ではなくeditor toolのpath引数および書き込みリダイレクト・書き込み系コマンドの対象に基づいて行い、ネストしたコマンドでも同じ判定に落とす（解析不能時は保守的にdeny）。2つのhookの契約が矛盾しないことを、lane起動形と`out/stop-report.json`書き込みの両方について回帰テストで固定する。Evidenceの合否権限とfail-closed境界は変えない |
| O-12 | 筐体laneのentrypointと発注policyがGD1固定で、GD1以外の設計がorder readiness判定に到達できない | `scripts/run_design_lanes.py`は`enclosure`に対し`scripts/run_gd1_enclosure_pipeline.py`（docstringは`Golden Design #1 mechanical pipeline`、実装は`src/acd/pipeline/gd1_enclosure.py`）を固定で呼ぶ。実機では`pulse-check-tag`に対し入口の`validate_and_project_rationale`で`rationale coverage failed: missing=94, stale=0, orphan=0, conflicting=0, unknown_provenance=0, untraceable=0, unclassified=0`（exit code 1）となった。graphに`mechanical.enclosure`・`mechanical.component_body`ノードがなく、`mechanical.outline`にも`mount_hole_count`がない。発注側も同様で、`scripts/aggregate_order_total.py`は`--quote-record`（`QuoteRecord`）と`OrderScope`を必須とするが設計fixtureから導出する経路が無く（実機で`error: the following arguments are required: --quote-record/--quote`）、`scripts/pre_order_gate.py`は`could not parse order total`で停止した。`plugins/acd/hooks/order-policy.json`は`required_evidence_ids`を`["evidence.gd1.electrical", "evidence.gd1.mechanical"]`、`design_graph_paths`を`["fixtures/golden-design-1/graph.json"]`で固定する | 高 | I-4 | 筐体laneのentrypointを設計非依存にし、必要な機械ノード・属性・rationaleをgraph宣言から導出する。不足はO-5と同じ入口で一括に機械可読として返す。`QuoteRecord`と`OrderScope`を設計fixtureとfab profileから生成する決定論的経路を用意し、見積実値が無い場合はfail-closedのままとしdummy値を生成しない。`order-policy.json`の`design_graph_paths`と`required_evidence_ids`を対象設計から解決する形式へ変える（禁止範囲、上限額、transmission検査は変えない）。GD1以外でもorder readiness判定（合格・不合格のいずれでも）が表示されることを回帰テストで固定する |
| O-13 | stop hookのrationale被覆検査がGD1固定パスだけを見て、対象設計と無関係に`pass`を表示する | `plugins/acd/hooks/scripts/check_rationale.py`は`GRAPH_PATH = "fixtures/golden-design-1/graph.json"`と`RATIONALE_PATH = "fixtures/golden-design-1/rationale.json"`を定数で持ち、`scripts/check_rationale.py`の既定引数も同じGD1パスである。実機では`pulse-check-tag`を対象に作業している間もstop hookが`Rationale coverage: pass`を出力し続けた一方で、同じ設計の筐体laneは`missing=94`でfail-closedしている | 中 | O-4 | hookの検査対象を会話で宣言された対象設計から解決し、解決できない場合は`not_applicable`と明示して`pass`という語を使わない。出力へ検査対象graphのパスとrevisionを含める。対象設計のrationaleが不足しているときhookが`pass`を出さないことをnegative testで固定する。停止権限とfail-closed境界は変えない |
