...
| AA-31 | placement投影が取付穴（M2×2）とアンテナkeepoutを描かず、部品bodyが基板外形をはみ出す場合（U1 上端約5mm、J1 下端約0.5mm）に意図的な張り出しかbody宣言の誤りかを注記しない | 同run `visual/dual-beacon-tag-placement.svg` | 低 | #389 | graphの`mount_hole`・keepout宣言を描き、外形外へ出るbodyには寸法付き注記（`overhang 5.0 mm (declared: antenna)`等）を付ける。宣言が無いはみ出しはL3警告として凡例へ載せる |
| AA-32 | KiCad層別SVG（F.Cu／B.Cu）は基板領域のみで題名・層名・板寸法・scale bar・表裏注記が無く、単独では層と向きを読めない。firmware-sequenceのlifelineは`comp.u1`等のidのみ、firmware-stateの遷移ラベルが近接線と重なる、power-treeのrail順がid順 | 同run `visual/dual-beacon-tag-{f-cu,b-cu,firmware-sequence,firmware-state,power-tree}.svg` | 低 | #389、#392 | KiCad層SVGをacd-svgのtitle・層名・寸法・scaleで包む。lifelineはrefdes＋value、遷移線は状態別にy段を分離、railは電圧降順（source→load）で並べる |
