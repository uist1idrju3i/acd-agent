...
Y-1・Y-4・Y-10はそれぞれの変更で解消した。Y-2（coverage要約）とY-7（silkscreen
resolverのfail-closed前倒し）も本変更で解消した。いずれも診断面の追加であり、合格側権限の緩和ではない。Y-10はEvidence provenance面の追加であり、合格側権限の
...
21. Y-1〜Y-11（第8回実機実測、自然文のみ新規設計）。Y-1・Y-4・Y-10・Y-2・Y-7は解消済み。残るうちY-11／Y-6（FW capability契約と要件→fw.sequence被覆検査）はfail-closed境界の堅持に直結し先に扱う。Y-8（router診断を`loop-summary`へ）はagentが止まったまま自力で切り分けるための診断面、Y-3（library hash採取）は反復コスト、Y-5・Y-9は宣言と文書の整備である。hook誤検出（N-3・N-6・N-8）は運用観測として別途扱う。
