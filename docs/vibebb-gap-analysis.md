...
O-1・O-9は「設計内容に依らず基板lane以降へ到達できない」直接原因、O-10はFW laneがGD1専用実装である直接原因、O-11はhook契約の矛盾、O-2は実行基盤側の律速であり、O-4・O-5は
...
13. O-1〜O-11（宣言経路解消後の`test5`実測）。O-1（`run_tool`のtimeout引数化）とO-9（pass予算既定の単一化）は基板lane到達の前提であり最優先。O-10（FW laneのGD1固定解消）とO-11（projection guardの誤検出と迂回）も同順位で扱う。次にO-2（container起動前のホスト資源検査）、O-5・O-4（一括preflightと語彙の是正）を扱う。O-3・O-6〜O-8は運用と手順の整備である。
