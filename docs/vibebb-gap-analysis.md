...
| O-12 | 筐体laneのentrypointと発注policyがGD1固定で、GD1以外の設計がorder readiness判定に到達できない | 発注pathは達成。`OrderPolicy`が固定graph path／Evidence IDではなく許可graph rootとEvidence laneを宣言し、pre-order gateが呼び出し側の対象graph pathをrepository境界・design input・parse・revision一致まで検証し、graph IDから必要Evidence IDを導出する。GD1以外のgraphでもauthoritative Evidenceが揃えば判定できる。筐体laneのentrypointはなお`scripts/run_gd1_enclosure_pipeline.py`固定で、機械宣言不足の診断も未解消 | 高 | I-4 | 筐体laneのentrypointを設計非依存にし、必要な機械ノード・属性・rationaleをgraph宣言から導出する。不足はO-5と同じ入口で一括に機械可読として返す。`QuoteRecord`と`OrderScope`を設計fixtureとfab profileから生成する決定論的経路を用意し、見積実値が無い場合はfail-closedのままとしdummy値を生成しない。発注pathのgraph-derived化は達成済みのため、entrypoint汎用化とは別PRで扱う |
