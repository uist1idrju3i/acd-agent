...
| AA-32 | KiCad層別SVG（F.Cu／B.Cu）は基板領域のみで題名・層名・板寸法・scale bar・表裏注記が無く、単独では層と向きを読めない。firmware-sequenceのlifelineは`comp.u1`等のidのみ、firmware-stateの遷移ラベルが近接線と重なる、power-treeのrail順がid順 | 同run `visual/dual-beacon-tag-{f-cu,b-cu,firmware-sequence,firmware-state,power-tree}.svg` | 低 | #389、#392 | 解消（part a）。firmware-sequenceのlifelineをrefdes＋valueとnode idの副ラベルで表示し、firmware-stateの遷移ラベルを交差線を避ける空き区間へ決定論的に配置し、power-treeを公称電圧降順（source→load）へ並べ替えた。KiCad F.Cu／B.Cu層SVGのacd-svgラッピングは次PRで実施する |
