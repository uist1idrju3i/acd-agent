...
| AA-28 | `record_visual_vision_observation.py`／`verify_visual_review.py`はobservationのhash一致と非空しか検査せず、`tool_name`・`model`はCLI引数の自己申告で、実際にvision toolのObservationEventが発生したかを照合しない。作文したobservationで`verify_visual_review.py`がexit 0になる | 同run 11:27〜11:28: 3件のobservationをfile_editorで作成→記録→`visual review: complete (3/3 observed)` | 高 | AA-27 | observation recordへ会話event id（vision toolのObservationEvent）とその応答hashを必須にし、`verify_visual_review.py`が会話event log（OpenHands API）またはhook記録と照合できない記録を`unverified`としてfail-closedにする。stop policy hookで`visual-observations/`配下への直接書き込みを`write_target`拒否へ加える |
...
| AA-33 | agent最終報告の`verify_authoritative_evidence.py`表がcontainer digestを`N/A`とし、Evidence本体の`envelope.container_image_digest`（`sha256:7ee77579…`）を読んでいない。報告契約が表の各列に一次資料path（JSON pointer）を要求しない | 同run `loop/agent-final-report.md` vs `loop/evidence-*.json` | 低 | Z-8、AA-10 | 報告契約で`status`・`target_revision`・`container_image_digest`・`source_revision`の4列を`verify_authoritative_evidence.py`の機械出力からそのまま引用させ、`N/A`・`unknown`を書く場合はその根拠path（欠落key）を添えさせる |
