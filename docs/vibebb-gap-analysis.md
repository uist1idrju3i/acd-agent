...
| AA-29 | #392の`select_paper`がgrid最悪値（列数`ceil(sqrt(n))`×行数×固定ピッチ）から用紙を選ぶため、17部品の回路図がA2で内容は左上約1/3に収まり、シンボルが用紙比で極小になる。net labelが部品value・pin名に重なる | 同run `visual/dual-beacon-tag-schematic.svg`（A2、空白率約65%） | 中 | #392 | 配置後の実extent（symbol bbox＋label）から用紙を選び、列ピッチをsymbol幅の関数にする。net labelをpin端から一定offsetで外側へ置き、value/pin名との衝突を検査する |
| AA-30 | build123dのCAD SVG（`mechanical-section`・`mechanical-interference`）は外形線と穴だけで、題名・断面位置・寸法・基板断面・干渉体の強調が無く、両者が同じ見え方になる。干渉「なし」も図から読めない | 同run `visual/dual-beacon-tag-mechanical-{section,interference}.svg`のDevin描画が視覚的に同一 | 中 | 8.4、#389 | CAD由来pathをacd-svgのtitle・legend・寸法・注記で包み、干渉体はハイライト、無ければ「no interference (clearance ≥ … mm)」注記を描く。byte-exact規則（`build123d-svg-v1`）は包んだ後のSVGで維持 |
