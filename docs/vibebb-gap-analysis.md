...
[`src/acd/core/knowledge/design_predicates.py`](../src/acd/core/knowledge/design_predicates.py)は、net名
...
| M-4 | 電池の充電・保護回路とEMC/ESDの設計述語 | `src/acd/core/knowledge/design_predicates.py`の`PREDICATE_CATALOG`は6件で、電源境界とdecoupling以外に充電・保護・電力バジェット・保護素子有無の判定を持たない。L-6でtopology templateと部品catalogの追加経路は宣言経由へ開いたが、規範的な契約と述語は追加していない | 中 | 16.2（バッテリ駆動）・16.3（EMC/ESD） | 述語の適用条件を14.2の契約registryで宣言し、宣言外はunknownとして停止側へ集約する。消費電流と容量の収支を宣言由来入力から決定論的に検査する。正負両方のテストを持ち、既存GD1の判定と正規化hashを変えない |
...
| N-5 | U1のIO-to-pad mappingを宣言経由で与えられない | `src/acd/core/knowledge/design_predicates.py`の`_u1_io_pads`は`cpl_rotation_pin_functions`／`cpl_rotation_pin_aliases`からGPIO→pad対応を解決し、一意解決できないと`unknown`になる。MPNが`ESP32-C3-MINI-1-N4`と確定していても実機では`strapping_pin: status='unknown' (U1 IO-to-pad mapping is missing or ambiguous)`で基板laneが停止した | 高 | N-1、14.2の契約registry | parts catalog entryへモジュールのpin function mapを持たせ、`part_request`での選択時にgraphへ決定論的に展開する。展開元をprovenanceへ記録する。catalogに無い部品は従来どおりunknownでfail-closedとする |
...
| O-3 | 長時間laneのbackground実行が手順として規定されていない | 達成。`scripts/run_in_workspace.py --log PATH`が`acd-lane-log 0.1` header（image・revision・command・started_at）を実行前に書き、stdout／stderrをlogへteeし、result footer（exit_code・image_digest・execution_context・failure_kind・finished_at）を`finally`で追記する。`src/acd/core/runtime/lane_log.py`のparseはversion不一致・footer欠落（中断）・非整数exit_codeをfail-closedとする。`docs/operations.md`へ同時1本のbackground実行手順とtail／grep確認を記録した。container laneをOpenHandsのtool呼び出しでforeground実行すると、O-2の再起動でtool結果ごと失われ停止理由が判別できない。実機では`nohup ... > logs/<lane>.log 2>&1 &`へ切り替えて初めて、再起動後もexit code・image digest・fail-closed理由をlogから復元できた | 中 | O-2 | 達成。`--log`経路でimage digest・revision・コマンドをlog先頭へ必ず記録する |
...
| O-8 | 実行記録の収集入口がlane logと実機workspaceを対象にしていない | 達成。`scripts/export_execution_records.py`が`.log`入力とdirectory内の`*.log`を`parse_lane_log`で読み、`to_execution_record()`（`log_type: lane_log`）経由で既存のallowlist抽出・秘匿化・漏洩拒否を適用する。生image参照は落としdigestだけを残す。中断log（footer欠落）はexit 2で拒否し、directory内は`*.json`・`*.log`を名前順に混在処理する。`docs/operations.md`へリモートworkspaceからのrsync取得とexport手順を記録した。N-12に対して追加された`scripts/export_execution_records.py`はexecution record JSONのallowlist抽出と秘匿化を行うが、本検証で唯一の記録だったbackground実行laneの`logs/*.log`（exit code、image digest、fail-closed理由）は入力に含まれず、実機workspaceからの取得も1件ずつの手作業になった | 低 | N-12 | 達成。lane logの構造化取り込みと取得手順を実装し、秘匿化と漏洩検出をそのまま適用する |
...
| V-8 | 資源計測wrapperが特定checkout前提で固定されている | 達成。`scripts/measure_lane_resources.py`がcheckout path、digest固定image、download対象、計測間隔を引数で受け、`run_in_workspace.py`をwrapして`/proc/stat`・`/proc/meminfo`・`docker stats`をintervalごとに記録する。計測はL3観測であり、以前の固定wrapperはcheckoutパス、download対象、image指定が固定で、別workspaceでの再実行にコピー改変が必要だった | 低 | 15.13 | 達成。引数化されたwrapperをrepository内へ置き、使い捨てshell scriptを不要とした |
...
| Z-2 | 作動 | coverage診断「unclassified attrs are not part of the rationale contract, so remove them from the design input or propose the contract change in a separate PR」。`src/acd/core/knowledge/rationale.py`の編集は0件（`report/workspace-git-changes.json`） |
...
| Z-8 | 部分作動 | 機械出力（`M contracts/parts-catalog.json`・`M src/acd/core/manufacturing/part_selection.py`、82行）は正確に引用。添えた説明「pipelineが自動登録」「内部キャッシュ更新」は虚偽（AA-10） |
...
`contracts/parts-catalog.json`へのentry追加と`src/acd/core/manufacturing/part_selection.py`のmessage編集を
...
第10回の設計入力をそのまま用いた実機regen run（2026-09-09、#389〜#392反映後、
[`examples/dual-beacon-tag-vps-20260908/regen-run/`](../examples/dual-beacon-tag-vps-20260908/regen-run/README.md)）
では3 laneが完走してEvidenceが`valid`となった後、#391の`visual-review-manifest`段が実機で初めて
実行され、2欠陥（投影集合directory基準、SVG正規化規則）でfail-closedした（#395で修正済み）。
同runのevent監査でAA-27〜AA-28（vision tool不在と自己申告observation）、人間向け投影のDevinレビューで
AA-29〜AA-33が判明した。
...
| AA-2 | SessionStart hookのlock探索が会話用project dir・plugin root・`/opt/acd`に限られ、`/acd:init`が登録したworkspace registryのlockを見ない | init・loop両会話で「Authoritative tools are unavailable inside the locked image」（Z-12残） | 低 | Z-12、G-1 | 解消。initが`ACD_WORKSPACE_REGISTRY`（既定`~/.openhands/acd/workspaces.json`）へworkspaceをupsertし、SessionStartが登録時刻の新しい順にlockを探索して解決pathを`additionalContext`へ記載する |
...
| AA-19 | `UncoveredGroundRegionsError`・DRC `starved_thermal`／`courtyards_overlap`がbbox・座標のみで、島を囲むfootprint／track、関与参照子、有効なレバー（`min_clearance_mm`のfab最小内の下限）を示さない。`min_island_area`はzoneへ未出力（AA-11） | 修正run fix6〜fix13の8 run（via格子・refill・TP追加・J1移動は無効、`min_clearance_mm` 0.15→0.12で解消、0.10はrouter不収束） | 高 | AA-11、Y-8 | 解消済み。`enclosing_refdes`・`nearby_tracks`・`levers`をGND島診断と`gnd-stitch-vias.json`へ追加し、DRC違反へ関与refdesと型別レバーを添え、`acd-placement-search` Skillへ接続子直下の島解消手順を追加した。閾値・fab最小は緩めない |
...
| AA-21 | 新規部品のcatalog entry追加に正規経路が無く、agentは`contracts/parts-catalog.json`の未commit編集を`--allow-dirty`で通した。footprint／symbol hashの算出手順も未定義 | 第10回のcatalog編集（AA-5・AA-8）。修正runではagent由来entryをDevinがcontainer内hashで検証しcommit | 高 | AA-5、AA-8 | 解消済み。`--pin-hashes`／`--pinned-entry-out`でdigest固定container内のhash採取とentry生成を行う`acd-contracts` Skillの宣言経路を追加し、`contracts/`へのcommitとして`report_final_basis.py`に現れる形にした |
| AA-22 | agentがlcsc番号から品名・packageを確認する手順を持たず、typo（D1 `C16224`）や色違い（D2 `C2290`=`KT-0603W`）が最終gateまで残る。catalogに無い部品（橙LED）を探す経路も無い | 修正run fix16でD1訂正、D2は正規番号を特定できず未解決 | 中 | AA-12 | 解消済み。宣言直後に`fetch_lcsc_footprint_orientation.py --expect-mpn`でrecordを取得し、`Manufacturer Part`・package・Supplier Partを構造化して照合する。exit code 2と`evidence.cpl_rotation.mpn_mismatch`の次手をSkillへ記録し、catalog外の探索はL2として結果を宣言＋取得recordに残す |
| AA-23 | pipelineは投影（gerber・drill・CPL/BOM CSV・STEP・3MF・SVG・theme-song MIDI）を「writerが書けたこと」と`hashes.json`のsha256でしか記録せず、writerと独立したreaderで形式（SMF chunk長・note対応、STEP header/footer、3MF zip CRC・model XML、RS-274X `M02*`終端、Excellon `M48`〜`M30`）を検査しない。theme-songはwriter内部の再読込（`render_checked_midi`）のみで、第三者parserでの検証が無い。利用者が受領ファイルを開けない場合に、生成不良か再生環境かを生成物側から判別できない | 修正run fix18の収録投影72件を`mido`・`midicsv`・`timidity`・`zipfile`・XML parserで事後検査し全件OK（[`fixed-run/projection-format-check.txt`](../examples/dual-beacon-tag-vps-20260908/fixed-run/projection-format-check.txt)）。利用者報告の`theme-song.mid`破損は3者sha256一致・構造正常で再現せず | 中 | — | 解消済み。投影直後に`projection-format-check.json`（L3、`pass_evidence=false`）を書き、独立readerのchecker名・版・要約値を各投影について記録する。既存consumer向けに`hashes.json`はflatのまま維持し、形式検査record自体のhashだけをhash manifestへ追加する。parse失敗はfail-closedでpipelineを停止し、検査成功はEvidence・fab package・ゲート判定へ作用させない。theme-song MIDIにはSMF独立parserを適用し、WAV renderはtools imageにsynthが無いため未実施とする |
| AA-24 | acd-agentが生成できる投影のうち、`acd-product-docs`（製品説明README・取扱説明書、roadmap 9.1〜9.2）、`scripts/verify_manufacturing_submission.py`（製造提出verdict、CIの`container-gates`のみ）、`derive_png_visual_projections`（PNG raster、testのみ）は`run_design_loop.py`／`/acd:vibebb-loop`から呼ばれず、loop完走後も利用者へ渡る投影集合に含まれない。利用者が「全投影」を求めても、agentは別CLIの存在と引数を自力で見つける必要がある | 修正run fix18は`--design-only` loop完走後も`out/docs/`・`manufacturing-submission.json`・`visual/png/`を持たなかった。Devinが同digest containerで`generate_product_readme.py`（2回実行でbyte一致）と`verify_manufacturing_submission.py`（`status: pass`、host `--verdict`再検査exit 0）を追加実行して初めて得られた。PNG rasterはCLI自体が無い | 中 | — | 解消済み。`run_design_loop`の3 lane完了後に`projection-docs`と`manufacturing-submission`段を実行し、`--design-only`でも`out/docs/`（2文書、provenance、flatな`hashes.json`）と`out/manufacturing-submission.json`（`require_authoritative=false`のhost provisional verdict）を生成する。PNG rasterは既存の`visual-review-manifest`段で`visual/png/`へ生成する。これらはL3投影／provisional観測であり、CIの`verify_manufacturing_submission.py`によるauthoritative再検査、Evidence、L1判定を置き換えない |
| AA-25 | `generate_instruction_manual.py`はGD1固有のmacro集合（`ACD_PIN_UART_TX/RX`・`ACD_PIN_USB_DP/DN`・`ACD_PIN_BOOT`・`ACD_SHT40_I2C_ADDRESS`・`ACD_LOG_PERIOD_MS`）を必須とし、GD1以外の設計では取扱説明書を生成できない | fix18の`acd_pins.h`（`LED`・`LED2`・`BUTTON`・`I2C_SDA`・`I2C_SCL`・`LED_BLINK_PERIOD_MS`）に対し旧必須macroが無くfail-closed（exit 1、`fixed-run/docs/instruction-manual.fail-closed.log`） | 解消済み。必須macroはrevisionだけに縮小し、firmware action、pin role、mechanical connector openingのgraph宣言から節を導出する。宣言済み入力のmacro欠落はfail-closedで止め、未宣言機能は推定せず文書末の`省略した項目`へ理由を記す。GD1実headerとDBT graphで二つのconnector、LED2、button操作、sensor／USB省略、revision mismatch、macro矛盾、byte一致を回帰固定した |
| AA-26 | theme-song投影はSMF（`.mid`）のみで、テキストで読める楽譜形式が無い。利用者の再生環境（QuickTime Player X・SoundFont未設定のVLC等）では`.mid`を開けず、内容を確認・編集する手段が生成物側に無い | 利用者報告: 収録`theme-song.mid`をQuickTime／VLCで再生できず、ブラウザ上のMIDI playerで再生できた（ファイルはbyte一致・構造正常、AA-23）。利用者からMML形式の投影出力の要望 | 解消済み。`acd-mml 0.1`を固定し、同一`Score`から`theme-song.mml`を決定論的に生成する。独立parserでtempo・総tick・note数・channelごとのnote列を照合し、不一致時はMMLだけを省略して`mml_check`へ理由を記録する。MMLは`.mid`と並べてprovenanceと`hashes.json`へ登録し、MIDI・gate・Evidence・fab packageへ影響させない |

| AA-27 | OpenHands Local GUIのconversationには`VisionInspectTool`（`inspect_image_with_vision`）が登録されず、#391が`vibebb-loop.md`で必須にした「manifest全entryをvisionで検査する」手順を実行する手段がagentに無い。tool一覧はterminal・file_editor・task_tracker・browser・delegate・finish・think・switch_profile・skillのみ | 実機regen run（2026-09-09、[`examples/dual-beacon-tag-vps-20260908/regen-run/`](../examples/dual-beacon-tag-vps-20260908/regen-run/README.md)）のSystemPromptEventにvision toolが無い。agentはPILで寸法を読むだけの本文をfile_editorで作文し、`--tool-name inspect_image_with_vision`として記録した | 高 | AA-24、roadmap 8.4 | 解消。SessionStart hookがSDK profile storeの場所と保存済みprofileをL3 contextへ報告し、visual-review-manifest直後にtool一覧へ`inspect_image_with_vision`が無ければPIL/file_editor代替を禁止して`vision_tool_unavailable`でfail-closed stopする。次手はvision-capable profileのSDK profile store登録と会話再起動であり、視覚観測を合格権限へ昇格させない |
| AA-28 | `record_visual_vision_observation.py`／`verify_visual_review.py`はobservationのhash一致と非空しか検査せず、`tool_name`・`model`はCLI引数の自己申告で、実際にvision toolのObservationEventが発生したかを照合しない。作文したobservationで`verify_visual_review.py`がexit 0になる | 同run 11:27〜11:28: 3件のobservationをfile_editorで作成→記録→`visual review: complete (3/3 observed)` | 高 | AA-27 | 解消。inspect_image_with_visionのPostToolUse hookが応答hash・profile・modelを含むevent logを書き、observationはeventへ一度だけbindする。欠落・改変・不一致は`unverified`としてvisual reviewをfail-closedにし、observation／event logの直接書き込みを拒否する。L3観測とverdictは合否権限を持たない |
| AA-29 | #392の`select_paper`がgrid最悪値（列数`ceil(sqrt(n))`×行数×固定ピッチ）から用紙を選ぶため、17部品の回路図がA2で内容は左上約1/3に収まり、シンボルが用紙比で極小になる。net labelが部品value・pin名に重なる | 同run `visual/dual-beacon-tag-schematic.svg`（A2、空白率約65%） | 中 | #392 | 解消。配置ピッチをsymbol extentとlabelから導出し、Reference/Valueをbody外へ置き、label衝突をfail-closedで検査し、用紙を実extentから選ぶ |
| AA-30 | build123dのCAD SVG（`mechanical-section`・`mechanical-interference`）は外形線と穴だけで、題名・断面位置・寸法・基板断面・干渉体の強調が無く、両者が同じ見え方になる。干渉「なし」も図から読めない | 同run `visual/dual-beacon-tag-mechanical-{section,interference}.svg`のDevin描画が視覚的に同一 | 中 | 8.4、#389 | raw ExportSVGを`svg#cad-view`としてacd-svg文書へbyte-exactに埋め込み、title・legend・寸法・基板外形・ゲート測定注記を付与する。干渉体はハイライトし、無ければ測定済みclearanceを注記する。`build123d-svg-v1`はラップ後のSVG hashへ適用する |
| AA-31 | placement投影が取付穴（M2×2）とアンテナkeepoutを描かず、部品bodyが基板外形をはみ出す場合（U1 上端約5mm、J1 下端約0.5mm）に意図的な張り出しかbody宣言の誤りかを注記しない | 同run `visual/dual-beacon-tag-placement.svg` | 低 | #389 | 解消。graphの取付穴・keepout宣言を描き、外形外へ出るbodyへ宣言値と要件または未宣言L3警告を寸法付き注記として付与する |
| AA-32 | KiCad層別SVG（F.Cu／B.Cu）は基板領域のみで題名・層名・板寸法・scale bar・表裏注記が無く、単独では層と向きを読めない。firmware-sequenceのlifelineは`comp.u1`等のidのみ、firmware-stateの遷移ラベルが近接線と重なる、power-treeのrail順がid順 | 同run `visual/dual-beacon-tag-{f-cu,b-cu,firmware-sequence,firmware-state,power-tree}.svg` | 低 | #389、#392 | 解消。firmware-sequenceのlifelineをrefdes＋valueとnode idの副ラベルで表示し、firmware-stateの遷移ラベルを交差線を避ける空き区間へ決定論的に配置し、power-treeを公称電圧降順（source→load）へ並べ替え、KiCad F.Cu／B.Cu／In*.Cu層SVGをacd-svgの題名・層向き・寸法・scale bar・注記付きで包んだ |
| AA-33 | agent最終報告のEvidence表がcontainer digestを一次資料から引用しない | 同run `loop/agent-final-report.md` vs `loop/evidence-*.json` | 低 | Z-8、AA-10 | 解消。検証scriptが4列のcitation行とJSONを出力し、欠落値はJSON pointer付きunknownで示す |
...
`profiles/`・image digest lock変更のPRでも`container-gates`で実行）、AA-11（KiCad zone
fillへの最小島面積設定出力）、AA-13（part_request無しcomponentへのCPL evidence revision投影と
unknown属性診断）、AA-14（未配置silkscreen textのmeasured pass後Skill探索）は実装した。
...
23. AA-1〜AA-33（第10回実機実測、14.23反映後。AA-11〜AA-26は修正後runの実測で、AA-27〜AA-33は実機regen run（2026-09-09）の実測で追記）。AA-26は`acd-mml 0.1`の追加と独立parser照合で解消済み。AA-27・AA-28（vision tool不在と自己申告observationで視覚レビュー必須段が形式的に通る面）は#391の契約を実効化するため優先度高。AA-5（`--allow-dirty`の範囲限定）とAA-8（catalog hash照合）はcontract変更が会話内の未commit編集で通った経路を閉じるため最優先。次にAA-3（bootstrap record不在の停止報告）でZ-3・Z-8を実効化し、AA-4・AA-6・AA-7（診断文とpin role導出）で宣言経路の停止境界を減らす。AA-11（zoneへの`min_island_area`未出力）とAA-12（宣言mpn／lcscとrecord内容の不一致の未検出）は修正runで実害を確認した宣言〜検証の乖離。AA-15〜AA-22は修正後runでDevinが人手で行った作業の振り返り（`vibebb-standalone-verification.md` 17.12）を出所とし、停止文への次手の付与とSkill手順の整備でagent単独の到達段を進める項目である。AA-17（spec先行の再生成の必須化）とAA-19（GND島診断）、AA-21（catalog追加経路）を優先する。AA-23（投影の独立reader形式検査）は利用者へ渡す投影の受領可否を生成物側から判別するためのL3記録で、投影段の直後に置く。AA-24（文書lane・製造提出verdict・PNG rasterのloop組み込み）とAA-25（取扱説明書生成のGD1固有macro依存の解消）は、loop完走後に利用者が受け取る投影集合をacd-agentの全投影へ揃える項目で、いずれもL3投影の追加であり判定を変えない。AA-1は別PRのfixture修正、AA-2はZ-12の続き、AA-9・AA-10・AA-13・AA-14はL3警告・診断・報告契約の整備である。
