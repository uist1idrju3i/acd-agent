...
coreは外部ツール固有の型、ファイル形式、座標系変換を持たない。ゲートの実行は外部ツールを
呼ぶ境界（adapter）に置くが、合否の基準は設計の入力ファイルと`profiles/`の宣言を正とし、
adapterが基準を自前で決めない。
...
配置・回転・配線探索では、探索器・整合化器・代理指標は`plugins/acd/skills/`のSkill（生成側）に
置き、投影・外部router等の実測・ゲートをACD本体（判定側）に置く。判定側は生成側へ依存せず、
生成側の候補や代理指標を合格根拠として直接信頼しないことで、生成と判定の分離を具体化する。
委譲の範囲は[`adr/ADR-0009-openhands-delegation-and-skills.md`](adr/ADR-0009-openhands-delegation-and-skills.md)を正とする。
