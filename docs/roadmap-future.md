...
- 長時間運用
- ローカル製造
- 高密度基板（16.1の後続（BGA escape・microvia））
- 複数利用者・複数instanceの同時運用要件が出た時点で検討
- OpenBlink（mruby/cとBLEによる無線コード差し替え）を用いたFW動作の反復探索
- FPGA（GOWIN等）への対応。FPGAロジック（HDL・ネットリスト）とビットストリーム生成を
  宣言由来の設計contractとして扱い、yosys／nextpnr-himbaechel等のOSS toolchain
  （ISC／MITライセンスを前提）で決定論的に投影・検証する。GOWIN EDA等のproprietary
  toolはsubprocess呼び出しに限定し、imageへインストールしない。導出できない割当・制約は
  unknownとしてfail-closedにする。
- 組み合わせて使用するPC側のソフト開発（WebGUI等）。完成した基板・製品と組み合わせて
  動作するPC側アプリ・WebGUIの生成。前提の機器I/F契約投影はマイルストーン9.6へ移行済み
- 組み合わせて使用するサーバ側のソフト開発（バックエンド・フロントエンド）。製品と連携する
  サーバ側ソフトの開発。前提の機器I/F契約投影はマイルストーン9.6へ移行済み
- 組み合わせて使用するスマホアプリ開発。製品と連携するスマホアプリの開発
  。前提の機器I/F契約投影はマイルストーン9.6へ移行済み
- 部品ライブラリ統治は17.1へ移行し、`acd-library-governance` Skillでfootprint geometry、
  library source、asset hash、project library tableをL2レビューとして検査する。規制認証は
  対象外である
- BOMコンプライアンス事前チェックは17.3へ移行し、RoHS等の申告状況を集計する。申告の
  集計は適合判定ではなく、未申告・不明・staleはunknownとして列挙する
- ワイヤハーネス設計。基板外配線（電線、ケーブル、FFC/FPC、コネクタ）を宣言由来の
  設計contractとして扱い、基板設計と同じ決定論的ゲートで検査する
- 信頼性試験（EMC、環境試験）と認証設計。試験規格への準拠確認にとどめず、規格が成立した
  背景と規格が模擬している実使用環境まで設計入力として宣言する。想定実使用環境の
  宣言contractはマイルストーン16.3の入力へ移行済み
...
第1段（結線・電線種別・許容電流の宣言contractと述語）はマイルストーン16.6で達成した。
`HarnessContract`、netlist consistency、許容電流・電圧降下・絶縁・曲げ半径gate、および
ハーネス図と切断長表の決定論的projectionを提供する。GD1のようにcontractを宣言しない設計の
既存出力は変更しない。
第2段として、シールド・撚り対、可動部屈曲、嵌合回数・保持力、DFA連携、信号クラス隔離、
16.5 SPOFのハーネス共有検査まで実装した。残る拡張は、測定Evidence境界と、より詳細な
shield termination・固定点・圧着・可動経路の機械モデルである。現行のlaneは基板と筐体を対象としており、基板外の配線は設計contractを持たない。取り込む
...
- 複数fab profileと製造データ契約の拡張 → マイルストーン14.2
- 分野横断の責務割当と構想ブラッシュアップ → マイルストーン21
- 多層・EMC/ESD・熱・SIの基盤範囲 → マイルストーン16.1・16.3、10.2・10.3
- routing後のvia mask開口を含む再解決 → マイルストーン14.25
- 長時間runの予算・中断・再開 → マイルストーン15.20
- 代替routerの単独実測 → マイルストーン15.21
- 機器I/F契約投影 → マイルストーン9.6
- ハーネス契約と結線検査 → マイルストーン16.6
- 想定実使用環境の宣言contract → マイルストーン16.3
- BOMコンプライアンス申告状況の集計 → マイルストーン17.3
- BOMコスト・代替部品候補の整理 → マイルストーン17.4
- ACD imageのコンテナ化と配布 → マイルストーン6・14.9
