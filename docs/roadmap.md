...
- 分野横断の責務割当（機構・機械・電気・ソフト・サーバを横断して機能の配置先を見極める）
...
分野横断の責務割当は、機構・機械・電気・ソフト・サーバを横断的に見て、どの機能を
どこ（ハードウェア、FW、PC側ソフト、サーバ、スマホアプリ）に持たせれば最適な体験を
提供できるかを見極め、責務を割り当てる構想である。割当の判断は設計入力・制約・rationaleを
出所とする決定論的な検討として扱い、SKILLの所見だけでは確定しない。割当結果はDesign Graph等の
設計入力contractへ宣言として記録し、投影を設計入力へ逆流させない不変条件を維持する。
導出できない割当はunknownとしてfail-closedで停止する。採用する場合は、各分野のcontract境界と
責務割当のEvidence境界を新規ADRで定義する。
