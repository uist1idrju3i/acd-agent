...
実装状況: AA-3は実装済み（bootstrap record不在の停止を`bootstrap_record_missing`宣言までdeny）、AA-4は実装済み（`PartSelectionError`へ要求内容と次手）、AA-5は実装済み（dirtyなsource treeは`--allow-dirty`でも拒否）、AA-6は実装済み（未知block診断へ登録名一覧を添え`requirement.block_missing`で停止）、AA-8は実装済み（graphの`parts_catalog_sha256`をcheckoutの契約と照合し`contract.hash_mismatch`で停止）。詳細は[`vibebb-gap-analysis.md`](vibebb-gap-analysis.md)のAA節を正とする。
