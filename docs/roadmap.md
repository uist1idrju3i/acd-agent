...
- 契約はPydanticモデルから導き、文書とスクリプトで必要なゲートを確認する。
- フェーズを実機測定の待ちで止めない。必要な実機テスト項目と結果を工程の記録へ反映する。
...
5. 最小記録（ツール名・版・入力hash・出力hash・収束状態・実行時刻）。
...
| Phase 13 AI主導の配置・回転探索 | 決定論的候補生成・合法化、代理指標による大域探索、少数候補の実測と決定論的ゲートをSkillとして提供する | 強化学習モデルの学習、LLMによるトレース生成 | 代理指標は候補順位付けだけに使い、実測は少数候補に限定でき、合否は投影と決定論的ゲートだけで決まる |
