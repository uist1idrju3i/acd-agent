...
| 14.14 | 宣言経路解消後の実機実測で残った不足（O-1、O-2、O-4、O-5、O-9〜O-13） | N-1・N-5の解消後に実機OpenHands（workspace `test5`）へ新規小規模設計`pulse-check-tag`を投入した実測で残った不足を扱う。`run_tool`のtimeout引数化とtimeout収束状態の区別（O-1）、routerのpass予算既定の単一定数化（O-9）、container起動前のホスト資源検査とdoctor項目追加（O-2）、lane別必須宣言の一括preflightをlane入口と同一述語で提供（O-5）、preflight語彙と診断限定の機械可読表明（O-4）。さらにFW laneの必要netと生成codeをgraph宣言由来へ移してGD1固定を解消（O-10）、projection guardの判定を生成物への書き込みに限定しlaneの出力先指定と`out/stop-report.json`の記録を許可（O-11）、筐体lane entrypointと発注policyのGD1固定解消とorder-total入力の導出経路（O-12）、rationale被覆検査の対象解決と`pass`語彙の限定（O-13）を扱う。判定と閾値は緩めず、timeout・unknown・未宣言を合格へ倒さない |
...
| （実機実測）O-5 lane別必須宣言の一括preflight | 14.14 |
