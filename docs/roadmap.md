...
筐体face契約は`front`・`back`・`left`・`right`の受理と
`mechanical.connector_opening.face_unsupported`のpreflight前倒しで解消済みである。
