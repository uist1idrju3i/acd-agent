...
| Phase 5 検証ゲートと根拠 | `small-production`以上で有効化する量産品質の機構として、多段検証、Evidence失効の伝播、影響導出、実機テスト項目の自動生成、投影レビューPDCAを実装する。機械可読投影と視覚投影を分類し、`ImageContent`／`inspect_image_with_vision`によるvisionレビューを修復ループへ接続する。 | 協調修復の自動化、長期知識loop、高精度SI・熱解析をこのフェーズへ含めること、およびこれらを`hobby`の最小構成へ持ち込むこと | Evidence失効伝播・影響導出・`ReviewFinding`処分状態・gate matrix・waiverを一意に実装し、未処分の重大findingやstale Evidenceを下流へ通さない。レビューの合否根拠は決定論的ゲートだけとする |
...
| Phase 13 AI主導の配置・回転探索 | `small-production`以上ではL1の機械可読な探索仕様宣言と探索固有Evidence、L2の決定論的候補生成・整合化・代理指標順位付け、L3の少数候補への実測と決定論的ゲートを実装する。全profileでLLMに座標・回転角を直接出力させず、代理指標を合格根拠にせず、実測は少数候補に限定する。Phase 5の検証ゲートと根拠、Phase 6の協調修復に依存する | 強化学習モデルの学習、LLMによる直接の座標・角度生成、LLMによるトレース生成 | `small-production`以上で探索仕様の機械可読契約と探索予算・停止理由のEvidenceを定義し、代理指標のみで大域探索し実測は少数候補に限定した探索ハーネスをリポジトリ内（`scripts/`または`packages/`）にcommitする。回転刻みの緩和がprofile許可とEvidenceに基づいて判定され、LLMなしの既定探索仕様との対照条件で効果を測定できる |
