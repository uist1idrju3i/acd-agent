...
| Phase 9 長時間ラン運用 | task ledger、checkpoint/resume、side-effect journal、予算、watchdog、golden task回帰 | 無制限retry、根拠なしの自動復旧 | 強制終了後に同じrevisionから再開して完走し、成果物hash・gate結果・event列・台帳が一致する。同一入力の外部副作用を重複させない |
