...
`sdk.logger`／`sdk.observability`、`sdk.settings`／`sdk.credential`／`sdk.profiles`まで
実装済みである。`sdk.context.memory`／`sdk.context.view`は未着手である。
...
| 4.4 | SDK機能移譲 | SDKのcontext、routing、保存、観測、設定、credential、profile、workspaceへ責務を段階移譲する | 一部実装（`sdk.context.prompts`、`sdk.llm.router`、`sdk.io`、`sdk.logger`／`sdk.observability`、`sdk.settings`／`sdk.credential`／`sdk.profiles`実装済み。context.memory以降は未着手） |
...
settings/profile資材のcanonical hash固定とcredential参照名だけの保持も実装済みである。一方、
