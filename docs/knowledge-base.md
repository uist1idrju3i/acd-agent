...
- **CPLの基準を独立に検証する。** 生成物と独立測定が同じ基準を共有していると、その基準自体の誤りは検出できません。fabが要求する位置・回転基準は一次情報で確認し、算出方法や部品番号ごとの向きが非明記なら`unknown`として発注を停止します。第三者の補正テーブルは出所として採用しません。fab側ビューワは近似表示でありえますが、独立実測との差の符号まで一致する場合は自データ側の欠陥を疑います。製造データ生成と発注可否は分離し、未確認のままでも生成物を出力して`order-readiness.json`で発注不可を記録します。
...
- **fab側ライブラリのパッド配置を照合Evidenceにする。** JLC/EasyEDAライブラリの番号付き
  ...
  0/90/180/270度をピン機能ごとの最大距離で比較し、一意の場合だけオフセットを確定する。
  これはメーカーのtape&reel図ではないため、`fab_library_footprint`として限界を明示し、
  第三者の補正表で補完しない。
  ただし照合キーはパッド番号ではなくピン機能名とする。ライブラリ間で番号の意味は
  異なり得るため、一意な幾何解だけでは正しさを証明できず、極性部品では逆実装を生む。
  正規化しても1:1対応できない部品はfail-closedとし、無極性・対称部品だけは極性に
  影響しない根拠をEvidenceへ残して幾何照合を許容する。
  ピン機能照合が第一優先であり、機能対応を省略した幾何照合は、機構的な向きや仕様上の
  機能対称性を出所付きで宣言した部品に限る。左右反転は回転では解消できないため、
  幾何が一意でも、左右の機能が電気的に交換可能であることを別途確認する。
  `cpl_rotation_pin_functions`の手書き宣言自体も単一障害点になるため、graphが宣言する
  symbol pathとsha256を検証してKiCad symbolからピン番号・ピン名を再抽出し、宣言値と
  一致することを独立検証する。symbolにないパッドは未検証パッドとして列挙し、極性に
  影響しない根拠と出所がなければ発注を停止する。
