...
OpenHands SDKはConversation、型付きTool、workspace、skills／plugin、subagent
（`AgentDefinition`）、hooks、critic、`/goal`、condenser、security analyzer／
`ConfirmationPolicy`、`AgentProfile`、workflow／taskを提供する実行基盤です。これらの
既存機能を優先してフル活用し、設計グラフ、決定論的ゲート、Evidenceの失効、承認IDと
不可逆操作の束縛はACDが実装します。
