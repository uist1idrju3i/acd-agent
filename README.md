...
基本サイクルにします。予算上限と発注前最終ゲートを満たせば、承認IDなしでfabへの発注まで
自働で進められる設計を目指します。実機で問題が出たときは、設計根拠を手がかりに対話で
...
- AIは候補を提案し、決定論的ツールが判定する。パーサー、制約ソルバー、DRC、
...
- 設計根拠（Design Rationale）を必ず残す。判断理由、比較した代替案、前提条件、既知の懸念を設計グラフに紐づける。
- 差分の影響を分析し、必要なゲートと試験を再実行する。
- 監査文書、Q7/N7図表、BOM、製造データもグラフから投影する。
- ファームウェアも投影と検証の対象とし、ビルド、静的解析、単体テスト、仮想実機シナリオ、
  実機ログの期待値照合を決定論的Evidenceとして判定する。
- 人間レビューは任意で、既定はAIが要件から製造データまで走り切る。ユーザーが確かめるのは
  回路図やアートワークではなく、届いた基板と筐体が実際に動き、収まるかどうかである。
  未知の影響や異常は合格扱いせず、不可逆操作には予算・最終ゲート・承認状態を適用する。
- Q7/N7を分析器として使い、知識を事実と測定から蓄積する。
- staleなEvidenceを下流へ流さず、外部ツールの版・入力・出力・不確実性を記録する。
- 承認された修正と却下された提案、fabからの指摘、試作の失敗をすべて構造化して記録する。

## 設計フロー

```mermaid
flowchart LR
    S1["S1 要件対話<br/>電気・機械・製造"] --> E2["S2 電気<br/>部品・回路"]
