...
ACDの中心命題は、**重い検証を全自働化することで人間の負荷をゼロにする**ことである。
VibeBBは設計や検証が軽いという意味ではなく、重い検証を人間に見せないことで、バイブスのまま
安全に設計を進められることを意味する。

## 想定ユーザー

作者自身が最初のユーザーであり、dogfoodingを前提とする。将来の対象は、KiCadは使えるが
基板1枚に週末を溶かしている個人開発者である。

## 設計プロファイル

設計プロファイルは、検証の重さをリスクに合わせて調整する第一級の概念である。

| プロファイル | 既定の扱い |
|---|---|
| `hobby` | 既定値。最小限の安全・整合ゲートを有効にする |
| `small-production` | `hobby`に加え、[`reliability-practices.md`](docs/reliability-practices.md)の要求を段階的に有効にする |
| `high-reliability` | 同文書の要求をより広く有効にし、追加の根拠・検証・変更管理を要求する |

プロファイルのテーラリングは、適用範囲、除外理由、代替根拠、残余リスクを記録して行う。
プロファイルごとの要求の正は [`docs/reliability-practices.md`](docs/reliability-practices.md) に置く。

## 成功の計測対象

数値目標は置かず、次の4項目を計測対象とする。数値目標はPhase 1到達後に実測から置く。
...
と表現できる。実機で問題が出たときは、設計根拠を手がかりに対話で
...
別リポジトリとしてそのまま開発を継続する。`acd-agent`はその後継・置き換えではない。
OpenHands連携でも同じコンセプトが上手く動くのではないかという着想から立ち上げた、
並走するリポジトリである。両リポジトリの仕様、ADR、フェーズ定義、教訓文は共有せず、
本リポジトリの権威は本リポジトリ内の文書に限る。
