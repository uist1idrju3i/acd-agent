...
def load_defect_document(path: Path) -> LoadedDefectDocument:
    ...
    try:
        ...
        raise DefectRecordError(
            f"defect document is invalid: {path}: {exc}"
        ) from exc
...
class DefectCheckResult(AcdModel):
    ...
    workaround_eligible: list[NonEmptyStr] = Field(
...
def compute_horizontal_scope(
    ...
    if criterion == "same_component_mpn":
        ...
        matches = [
            node
            for node in graph.nodes
            if node.id not in anchor_ids and node.kind in kinds
        ]
        ...
        matches = [
            ...
            if node.id not in anchor_ids
            and node.kind
            in {"fab.order_intent", "fab.process_allowance", "electrical.board"}
...
def check_defect_records(
    graph: DesignGraph, document: DefectDocument
) -> DefectCheckResult:
    ...
    for record in document.records:
        ...
        referenced.update(
            ...
            for node_id in scope.matched_node_ids
        )
        referenced.update(
            exclusion.node_id
            for scope in record.horizontal_scopes
            for exclusion in scope.excluded
        )
        ...
        if unknown_nodes:
            record_findings.append(
                _finding(
                    record.defect_id,
                    "unknown_node",
                    "defect record references unknown graph nodes: "
                    + ", ".join(unknown_nodes),
                    unknown_nodes,
                )
            )
