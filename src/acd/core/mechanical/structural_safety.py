...
def _is_connector(node: GraphNode) -> bool:
    ...
    return (isinstance(refdes, str) and refdes.upper().startswith("J")) or any(
        isinstance(value, str) and value.startswith("Connector") for value in (footprint, symbol)
    )
...
def _resource_ids(
    ...
    component_ids = {
        pin.component_id for net_id in net_ids for pin in lane.pins if pin.net_id == net_id
    }
    ...
    if resource == "power_bus":
        return {
            net_id for net_id in net_ids if _attrs(graph, net_id).get("power_rail") is True
        }, True
    if resource == "ic":
        return {
            component_id
...
def evaluate_single_point_of_failure(graph: DesignGraph, lane: ElectricalLane) -> PredicateResult:
...
def evaluate_protection_selectivity(graph: DesignGraph, lane: ElectricalLane) -> PredicateResult:
    protections = _protection_components(graph)
    required_rails = [
        node
        for node in _nodes(graph, "electrical.net")
        if node.attrs.get("power_rail") is True and node.attrs.get("requires_protection") is True
    ]
    if not protections:
        if required_rails:
            return _result(
    ...
    missing_trip = [
        node.id for node in protections if _number(node.attrs.get("trip_current_a")) is None
    ]
    if missing_trip:
        return _result(
            "protection_selectivity",
...
def _component_net_classes(graph: DesignGraph, lane: ElectricalLane, component_id: str) -> set[str]:
    return {
...
def evaluate_signal_class_segregation(graph: DesignGraph, lane: ElectricalLane) -> PredicateResult:
...
def evaluate_sneak_path(graph: DesignGraph, lane: ElectricalLane) -> PredicateResult:
    ...
    critical_components = {
        pin.component_id for net in critical for pin in lane.pins if pin.net_id == net.id
    }
...
def _copper_um(graph: DesignGraph, lane: ElectricalLane, net: GraphNode) -> float | None:
    ...
    if board is None:
        return None
    return _number(board.attrs.get("copper_um") or board.attrs.get("outer_copper_thickness_um"))
...
def evaluate_trapezoid_current_capacity(
    ...
    for net in targets:
        ...
        if capacity < current:
            failures.append(f"{net.id} capacity {capacity:.3f} A is below required {current:.3f} A")
    if failures:
        return _result(
            "trapezoid_current_capacity",
            "fail",
