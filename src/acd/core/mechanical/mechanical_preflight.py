...
def _rationale_findings(graph: DesignGraph, fixture_dir: Path) -> list[RequirementFinding]:
    ...
    try:
        document = RationaleDocument.model_validate(read_json(rationale_path))
    ...
    for node in graph.nodes:
        if node.kind == "mechanical.outline" and (
            ...
            if node.kind == "mechanical.connector_opening":
                ...
                if isinstance(face, str) and face and face not in SUPPORTED_OPENING_FACES:
                    add(
                        _finding(
                            "mechanical.connector_opening.face_unsupported",
...
def check_mechanical_preflight(graph: DesignGraph, fixture_dir: Path) -> MechanicalPreflightReport:
