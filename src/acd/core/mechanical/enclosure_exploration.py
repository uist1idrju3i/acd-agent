...
def _load_graph(path: Path) -> DesignGraph:
    try:
        ...
        raise EnclosureExplorationError(f"graph is invalid or unreadable: {path}: {exc}") from exc
...
def _enclosure_node(graph: DesignGraph) -> GraphNode:
    ...
    if len(nodes) != 1:
        raise EnclosureExplorationError("graph must contain exactly one mechanical.enclosure node")
    return nodes[0]
...
def _numeric_attr(node: GraphNode, attr: str) -> float:
    ...
    if isinstance(value, bool) or not isinstance(value, int | float):
        raise EnclosureExplorationError(f"{node.id}: attr {attr!r} is missing or not numeric")
    return float(value)
...
def validate_enclosure_dimensions(
    ...
    if non_mechanical:
        raise EnclosureExplorationError(
            "candidate references non-mechanical change dimensions: " + ", ".join(non_mechanical)
        )
    disabled = sorted(
...
def _candidate_values(
    ...
    if current_value < minimum or current_value > maximum:
        raise EnclosureExplorationError(f"{dimension_id}: graph value is outside declared bounds")
...
def enumerate_enclosure_candidates(
    ...
    *,
    ...
    for dimension_id in selected:
        ...
        if attr is None:
            raise EnclosureExplorationError(
                f"{dimension_id}: searchable mechanical dimension has no graph attribute mapping"
            )
        ...
        candidates.append(
            EnclosureExplorationCandidate(
                ...
                provenance={
                    ...
                    "script_name": "src/acd/core/mechanical/enclosure_exploration.py",
...
def _replace_node_attrs(
    ...
    nodes = [
        node.model_copy(update={"attrs": {**node.attrs, **updates}}) if node.id == node_id else node
...
def _apply_candidate(
    ...
    updates: dict[str, AttrValue] = {
        _DIMENSION_ATTRS[dimension_id]: value for dimension_id, value in candidate.changes.items()
    }
