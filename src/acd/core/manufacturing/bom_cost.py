...
from acd.core.manufacturing.bom import build_bom, group_bom_rows_by_mpn
...
def _price_for_quantity(
    ...
    applicable = [
        price_break for price_break in entry.price_breaks if price_break.min_qty <= quantity
    ]
    if not applicable:
        return None
...
def estimate_bom_cost(
    ...
    for mpn in sorted(rows_by_mpn):
        ...
        lines.append(
            BomCostLine(
                ...
                extended_minor=(
                    selected_prices[mpn] * extended_qty if mpn in selected_prices else None
                ),
    ...
    unknown_mpns = sorted(set(missing) | set(unusable))
    coverage_pct = 100.0 * len(valid_extended) / len(non_empty_mpns) if non_empty_mpns else 100.0
    partial_total_minor = sum(selected_prices[mpn] * valid_extended[mpn] for mpn in valid_extended)
    ...
    for mpn, line in line_by_mpn.items():
        if total_minor is not None and line.extended_minor is not None:
            line_by_mpn[mpn] = line.model_copy(
                update={
                    "share_pct": (100.0 * line.extended_minor / total_minor if total_minor else 0.0)
                }
            )
    lines = [line_by_mpn.get(line.mpn, line) for line in lines]
    ...
    if total_minor is not None and max_share is not None:
        for line in lines:
            if line.mpn != "no_mpn" and line.share_pct is not None and line.share_pct > max_share:
                warnings.append(
                    BomCostWarning(
    ...
    drivers = [
        ...
        for line in sorted(
            (line for line in lines if line.mpn != "no_mpn" and line.extended_minor is not None),
    ...
    if lifecycle_registry is not None:
        lifecycle_entries = {entry.mpn: entry for entry in lifecycle_registry.entries}
        for mpn in non_empty_mpns:
            lifecycle_entry = lifecycle_entries.get(mpn)
            if lifecycle_entry is None:
                continue
            qty = sum(len(row.refdes) for row in rows_by_mpn[mpn]) * price_book.build_quantity
...
def bom_cost_markdown(estimate: BomCostEstimate) -> str:
    ...
    for driver in estimate.cost_drivers:
        lines.append(f"- `{driver.mpn}`: {driver.extended_minor} ({driver.share_pct:.2f}%)")
    lines.extend(["", "## 代替候補", ""])
    ...
    if estimate.warnings:
        for warning in estimate.warnings:
            lines.append(f"- `{warning.mpn}`: {warning.reason} ({warning.share_pct:.2f}%)")
    else:
        lines.append("- なし")
    return "\n".join(lines) + "\n"
