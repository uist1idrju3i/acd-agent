...
def sha256_of_asset(path: Path) -> str:
    try:
        ...
        raise LibraryAssetError(f"library asset is unavailable: {path}: {exc}") from exc
...
def materialize_library_assets(graph: DesignGraph, fixture_dir: Path) -> tuple[dict[str, str], ...]:
...
def verify_fixture_library_assets(graph: DesignGraph, fixture_dir: Path) -> tuple[Path, ...]:
    """Verify that one fixture resolves and matches every declared asset."""
    return tuple(
        verify_fixture_library_asset(asset, fixture_dir) for asset in graph_library_assets(graph)
    )


def verify_materialized_library_assets(graph: DesignGraph, fixture_dir: Path) -> tuple[Path, ...]:
    ...
    for asset in graph_library_assets(graph):
        ...
        if not local.is_file():
            raise LibraryAssetError(f"declared library asset is missing from the fixture: {local}")
