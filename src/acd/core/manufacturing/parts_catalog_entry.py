...
def _read_entry_input(
    ...
    if isinstance(value, Path):
        ...
        if not isinstance(payload, dict):
            raise PartsCatalogEntryError(f"parts catalog entry is not a JSON object: {source}")
    ...
    if stripped.startswith("{"):
        try:
            ...
            raise PartsCatalogEntryError(f"parts catalog entry JSON is invalid: {exc}") from exc
        if not isinstance(payload, dict):
            raise PartsCatalogEntryError("parts catalog entry JSON is not an object")
    ...
    raise PartsCatalogEntryError(
        "parts catalog entry is neither a JSON object nor a readable file: " + str(value)
    )
...
def _pin_library_hashes(
    ...
    for kind, file_key, digest_key in (
        ...
        if not isinstance(declared_path, str):
            raise PartsCatalogEntryError(
                f"parts catalog entry library_ref.{file_key} must be a string when pinning hashes"
            )
        try:
            resolved_path = resolve_library_asset(declared_path)
            computed = sha256_of_asset(resolved_path)
        except LibraryAssetError as exc:
            raise PartsCatalogEntryError(f"{kind} library file cannot be pinned: {exc}") from exc
...
def _catalog_json(existing: str, entry: PartCatalogEntry) -> str:
    ...
    if entries_key < 0 or opening < 0:
        raise PartsCatalogEntryError("parts catalog has no recognized entries array representation")
    depth = 0
    in_string = False
    escaped = False
    closing = -1
...
def _write_catalog(
    ...
    try:
        ...
        raise PartsCatalogEntryError(f"parts catalog generated payload is invalid: {exc}") from exc
    ...
    try:
        ...
        raise PartsCatalogEntryError(f"parts catalog cannot be written: {path}: {exc}") from exc
