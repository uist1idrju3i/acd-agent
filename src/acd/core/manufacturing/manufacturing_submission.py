...
def evaluate_manufacturing_submission(
    *,
    ...
    def required_artifacts() -> str:
        missing = [
            name for name, path in paths.items() if not path.is_file() or path.stat().st_size == 0
        ]
        missing.extend(
            name
            for name in required_enclosure_names
        ...
        missing.extend(
            name for name in required_enclosure_names if name not in declared_enclosure_names
        )
        try:
    ...
    gerber_paths = [path for path in paths.values() if path.suffix.lower() in _GERBER_SUFFIXES]
    ...
    def normalized_hashes() -> str:
        ...
        for entry in enclosure_entries:
            ...
            if (
                ...
                or _artifact_hash(path, str(entry.get("format", _format_for(path)))) != expected
            ):
                failures.append(str(entry.get("path")))
        if failures:
    ...
    def geometry() -> str:
        ...
        if not isinstance(tolerance_obj, (int, float)) or float(tolerance_obj) <= 0:
            raise ManufacturingSubmissionError("graph board width measurement tolerance is missing")
        ...
        if not math.isclose(measured[0], expected[0], abs_tol=tolerance) or not math.isclose(
            measured[1], expected[1], abs_tol=tolerance
        ):
        ...
        if not math.isclose(measured[0], float(size["X"]), abs_tol=tolerance) or not math.isclose(
            measured[1], float(size["Y"]), abs_tol=tolerance
        ):
            raise ManufacturingSubmissionError(
    ...
    def revision_consistency() -> str:
        ...
        for name in ("evidence-electrical.json", "evidence-mechanical.json"):
            evidence_path = (
                board_dir / name if name == "evidence-electrical.json" else enclosure_dir / name
            )
