...
from acd.core.manufacturing.quote import read_quote
...
def order_total_result_to_document(
    ...
    expected_hash = order_total_breakdown_hash(
        quote_hashes=[(item.quote_id, item.canonical_hash) for item in result.quote_hashes],
        subtotals=[(item.category, item.amount) for item in result.subtotals],
        target_revision=result.target_revision,
        total=result.total,
    )
...
def order_total_result_from_document(
    ...
    expected_hash = order_total_breakdown_hash(
        quote_hashes=[(item.quote_id, item.canonical_hash) for item in document.quote_hashes],
        subtotals=[(item.category, item.amount) for item in document.subtotals],
    ...
    return OrderTotalResult(
        subtotals=tuple(
            OrderSubtotal(category=item.category, amount=item.amount) for item in document.subtotals
        ),
        total=document.total,
        target_revision=document.target_revision,
        quote_hashes=tuple(
    ...
    *,
    ...
    if missing_categories:
        raise OrderTotalError(
            "order scope required categories are missing: " + ", ".join(sorted(missing_categories))
        )
    ...
    for category, treatment in (
        ...
        if treatment != "itemized" and present:
            raise OrderTotalError(f"order scope does not permit a {category} fee item")

    if scope.mechanical_treatment == "included":
        expected_ids = set(scope.mechanical_item_ids or ())
        if mechanical_ids != expected_ids:
            raise OrderTotalError("mechanical fee item identifiers do not match order scope")
    elif mechanical_ids:
    ...
    if declared_total_minor != subtotal_total:
        raise OrderTotalError("quote declared totals do not match order fee subtotals")
    total = QuoteAmount(
    ...
    breakdown_hash = order_total_breakdown_hash(
        quote_hashes=[(item.quote_id, item.canonical_hash) for item in ordered_quote_hashes],
