...
def pinned_cern_commit(root: Path) -> str:
    ...
    try:
        ...
        raise CernCatalogError(f"CERN source pin is missing: {readme}") from exc
    match = re.search(
...
def cern_checkout_commit(root: Path) -> str:
    ...
    if actual != expected:
        raise CernCatalogError(f"CERN submodule commit drift: expected {expected}, got {actual}")
    return actual
...
def resolve_cern_part(root: Path, part_number: str) -> ResolvedCernPart:
    ...
    try:
        try:
            ...
            for table in tables:
                ...
                rows.extend((table, row[0], row[1]) for row in matches)
        except sqlite3.Error as exc:
    ...
    if len(distinct) > 1:
        mappings = ", ".join(f"({symbol!r}, {footprint!r})" for symbol, footprint in distinct)
        raise CernCatalogError(
            f"CERN catalog part {part_number!r} has conflicting library mappings: {mappings}"
        )

    raw_symbol, raw_footprint = distinct[0]
    symbol_library, _symbol_name = _split_library_ref(raw_symbol, "LibSymbol")
    footprint_library, footprint_name = _split_library_ref(raw_footprint, "LibFootprint")
    symbol_path = checkout / "SchLib" / f"{symbol_library}.kicad_sym"
    footprint_path = (
        checkout / "PcbLib" / f"{footprint_library}.pretty" / f"{footprint_name}.kicad_mod"
    )
    ...
    return ResolvedCernPart(
        ...
        symbol_file=str(CERN_SUBMODULE / "SchLib" / f"{symbol_library}.kicad_sym"),
