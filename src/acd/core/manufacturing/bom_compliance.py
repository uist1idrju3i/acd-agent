...
def _is_stale(
    ...
    *,
    ...
    return (
        ...
        or (declaration.source.valid_until is not None and declaration.source.valid_until < as_of)
    )


def _rows_for_mpn(rows_by_mpn: dict[str, tuple[BomRow, ...]], mpn: str) -> tuple[BomRow, ...]:
...
def summarize_bom_compliance(
    ...
    if graph.graph_id != registry.graph_id or graph.revision != registry.revision:
        raise ValueError("compliance registry graph_id/revision does not match graph")
    ...
    required_summaries = [summary for summary in regime_summaries if summary.regime in required]
    ...
    for mpn in governed_mpns:
        ...
        per_part.append(
            BomCompliancePartSummary(
                ...
                mpn=mpn,
                statuses={regime: status for regime, status in sorted(part_statuses[mpn].items())},
                msl_levels=sorted(
                    {
    ...
    if empty_rows:
        per_part.append(
            BomCompliancePartSummary(
                refdes=sorted({refdes for row in empty_rows for refdes in row.refdes}),
                mpn="no_mpn",
...
def compliance_summary_markdown(summary: BomComplianceSummary) -> str:
    ...
    for regime in summary.per_regime:
        if regime.unknown_parts:
            lines.append(f"- `{regime.regime}`: {', '.join(regime.unknown_parts)}")
    ...
    for regime in summary.per_regime:
        if regime.non_compliant_parts:
            lines.append(f"- `{regime.regime}`: {', '.join(regime.non_compliant_parts)}")
