...
from acd.core.manufacturing.fab import extract_fab_intent, load_fab_profile_registry
...
def _load_graph(fixture_dir: Path) -> DesignGraph:
    ...
    try:
        ...
        raise OrderScopeDerivationError(f"graph is invalid or unreadable: {path}: {exc}") from exc


def _load_rationale_revision(fixture_dir: Path) -> str:
    path = fixture_dir / "rationale.json"
    try:
        document = RationaleDocument.model_validate_json(path.read_text(encoding="utf-8"))
    except (OSError, ValueError) as exc:
...
def _load_terms(fixture_dir: Path) -> OrderTermsDeclaration:
    ...
    try:
        return OrderTermsDeclaration.model_validate_json(path.read_text(encoding="utf-8"))
    except (OSError, ValueError) as exc:
        raise OrderScopeDerivationError(
...
def derive_order_scope(fixture_dir: Path, *, registry_path: Path | None = None) -> OrderScope:
    ...
    try:
        ...
        raise OrderScopeDerivationError(f"fab profile registry is invalid: {exc}") from exc
    entries = [entry for entry in registry.document.profiles if entry.profile_id == fab_profile_id]
    if len(entries) != 1:
    ...
    if has_enclosure:
        ...
        if exclusion_reason is None:
            raise OrderScopeDerivationError(
                "no mechanical enclosure node; order terms must declare mechanical_exclusion_reason"
            )
    ...
    try:
        ...
        raise OrderScopeDerivationError(f"derived order scope fails validation: {exc}") from exc
...
def build_quote_request(scope: OrderScope, *, fixture_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "quote_record": None,
        "reason": (
            "QuoteRecord requires supplier quote values; none are derived from design inputs"
        ),
