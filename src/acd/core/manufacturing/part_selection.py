...
def select_part(
    ...
    if request.preferred_part_number is not None:
        entries = [entry for entry in entries if entry.part_number == request.preferred_part_number]
    if not entries:
        detail = f" for kind={request.kind!r} value={request.value!r} package={request.package!r}"
        if request.preferred_part_number is not None:
