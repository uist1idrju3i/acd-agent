...
def apply_input_feedback(
    ...
    *,
    ...
    updated_bytes = (
        json.dumps(graph_value, ensure_ascii=False, indent=2, sort_keys=True).encode() + b"\n"
    )
    before_hashes = {
        str(path): "sha256:" + hashlib.sha256(data).hexdigest() for path, data in originals.items()
    }
...
def _classification_is_known(node: GraphNode, attr: str) -> bool:
    return attr in REQUIRED_RATIONALE_ATTRS.get(
        node.kind, frozenset()
...
def propose_input_feedback(
    ...
    for rule in policy.rules:
        ...
        if not math.isfinite(measured_value):
            return _unknown_proposal(f"feedback measurement is not finite: {rule.measurement_name}")
        ...
        proposed_value = measured_value if rule.rule_kind == "set_value" else current_value
        rationale_required = (
            changed
