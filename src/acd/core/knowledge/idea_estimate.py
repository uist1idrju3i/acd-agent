...
def load_estimate_catalog(path: Path) -> IdeaEstimateCatalog:
    ...
    try:
        return IdeaEstimateCatalog.model_validate_json(path.read_text(encoding="utf-8"))
    except (OSError, ValueError) as exc:
        raise IdeaEstimateError(f"estimate catalog {path} is not valid: {exc}") from exc
...
def _finding(
    ...
    if field.unit != expected_unit:
        return EstimateFinding(
            constraint=name,
            status="not_comparable",
            detail=(f"{name} constraint unit {field.unit!r} is not {expected_unit!r}"),
        )
    limit = float(value)
    ...
    if low > limit:
        return EstimateFinding(
            ...
            status="stop",
            detail=(
                f"estimated {name} lower bound {low} exceeds the confirmed limit {limit}{suffix}"
            ),
        )
    if high > limit:
        return EstimateFinding(
    ...
    return EstimateFinding(
        constraint=name,
        status="within",
        detail=f"estimated {name} upper bound {high} within the confirmed limit {limit}{suffix}",
    )


def estimate_idea(record: IdeaRecord, catalog: IdeaEstimateCatalog) -> IdeaRoughEstimate:
    ...
    for function in record.functions:
        ...
        if entry is None:
            unknown.append(
                UnknownFunction(
                    function_id=function.function_id,
                    reason=(f"function_class {function.function_class!r} not in catalog"),
                )
            )
            continue
        lines.append(
