...
def load_promotion_rationale(path: Path) -> IdeaPromotionRationale:
    ...
    try:
        return IdeaPromotionRationale.model_validate_json(path.read_text(encoding="utf-8"))
    except (OSError, ValueError) as exc:
        raise IdeaPromotionError(f"promotion rationale {path} is not valid: {exc}") from exc
...
def _promoted_items(record: IdeaRecord) -> list[tuple[str, IdeaField]]:
    ...
    items.extend(
        (item.function_id, item.description) for item in record.functions if item.priority == "must"
    )
    items.extend(
    ...
    *,
    ...
    if open_items:
        raise IdeaPromotionError(
            f"idea {record.idea_id} has open items and cannot be promoted: " + ", ".join(open_items)
        )
    ...
    for requirement_id in by_id:
        if requirement_id not in known_ids:
            raise IdeaPromotionError(f"rationale record for unknown requirement {requirement_id!r}")
    for requirement_id, _ in pairs:
        if by_id.get(requirement_id, 0) != 1:
            raise IdeaPromotionError(
                f"requirement {requirement_id!r} must have exactly one rationale record"
            )
