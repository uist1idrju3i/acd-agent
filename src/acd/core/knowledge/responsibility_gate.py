...
def load_responsibility_declaration(path: Path) -> ResponsibilityDeclaration:
    ...
    try:
        return ResponsibilityDeclaration.model_validate_json(path.read_text(encoding="utf-8"))
...
def _finding(code: ResponsibilityFindingCode, subject: str, detail: str) -> ResponsibilityFinding:
    ...
    for field, need_value, cap_value in pairs:
        ...
        if field in {"gpio_count", "memory_kb"}:
            if (
                ...
                conflict = (
                    f"{field} need {need_value} exceeds domain {domain} capability {cap_value}"
                )
            ...
            if isinstance(need_value, list) and isinstance(cap_value, list):
                ...
                if missing:
                    conflict = f"communication {missing} not declared by domain {domain}"
        elif field == "power_source":
            if isinstance(cap_value, list) and need_value not in cap_value:
                conflict = f"power_source {need_value!r} not declared by domain {domain}"
        elif field == "motion" and need_value is True and cap_value is False:
            conflict = f"domain {domain} declares no motion capability"
        if conflict is not None:
            findings.append(("capability_conflict", f"{subject}.{field}", conflict))
    return findings
...
def check_responsibility(
    ...
    if declaration.graph_id != graph.graph_id or declaration.revision != graph.revision:
        findings.append(
            _finding(
                "graph_mismatch",
    ...
    for node in graph.nodes:
        ...
        by_function.setdefault(function_id, []).append((node.id, typed_domain, typed_criteria))

    shared = {item.function_id: set(item.domains) for item in declaration.shared_assignments}
    for function_id in sorted(needs):
        ...
        for node_id, domain, criteria in entries:
            ...
            for code, subject, detail in _capability_conflicts(need, capability):
                findings.append(
                    _finding(
    ...
    for dependency in declaration.dependencies:
        ...
        for source in sorted(left):
            for target in sorted(right):
                ...
                if not any(interface.domains == pair for interface in declaration.interfaces):
                    findings.append(
                        _finding(
                            "missing_interface",
