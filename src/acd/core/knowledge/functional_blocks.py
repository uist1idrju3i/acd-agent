...
def validate_predicate_coverage(
    ...
    required = {
        predicate for contract in registry.contracts for predicate in contract.required_predicates
    }
    ...
    if unknown or uncovered:
        ...
        if uncovered:
            details.append(
                "catalog predicates absent from registry contracts: " + ", ".join(uncovered)
            )
...
def declared_functional_blocks(
    ...
    for node in declarations:
        ...
        if not node.depends_on or any(
            dependency not in known_nodes or known_nodes[dependency].kind != "requirement"
...
def block_path(graph: DesignGraph, node_id: str) -> tuple[str, ...]:
    ...
    blocks = {node.id: node for node in graph.nodes if node.kind == "design.functional_block"}
...
def remediation_declarations(
    ...
    if unknown_blocks:
        raise FunctionalBlockContractError(
            unknown_block_message("functional blocks are not registered", unknown_blocks, registry)
        )
    source_blocks = tuple(
        block_id for block_id in declared if predicate in contracts[block_id].required_predicates
    )
