...
from acd.core.electrical.gpio import GpioAssignmentError, apply_gpio_assignment
...
def _load_record(path: Path) -> RequirementRecord:
    try:
        ...
        raise RequirementCompilationError(f"updated requirement is invalid: {path}: {exc}") from exc
...
def _update_requirement_nodes(
    ...
    try:
        ...
        raise RequirementCompilationError(f"requirement graph node is missing: {node_id}") from exc
    updated = [
...
def _update_coupled_labels(
    ...
    for node in graph.nodes:
        ...
        if (
            ...
            text = attrs.get("text")
            if isinstance(text, str) and (f"IO{old_gpio}" in text or f"GPIO{old_gpio}" in text):
                attrs["text"] = text.replace(f"IO{old_gpio}", f"IO{gpio}").replace(
                    f"GPIO{old_gpio}", f"GPIO{gpio}"
                )
                updated.append(node.model_copy(update={"attrs": attrs}))
                changed.append(node.id)
            else:
...
def _refresh_rationale(graph: DesignGraph, document: RationaleDocument) -> RationaleDocument:
    try:
...
def _add_requirement_node(
    ...
    if any(node.id == node_id for node in graph.nodes):
        raise RequirementCompilationError(f"requirement graph node already exists: {node_id}")
    ...
    last_requirement = max(
        (index for index, item in enumerate(graph.nodes) if item.kind == "requirement"),
...
def _delete_requirement_node(
    ...
    try:
        ...
        raise RequirementCompilationError(f"requirement graph node is missing: {node_id}") from exc
    dependents = sorted(node.id for node in graph.nodes if node_id in node.depends_on)
    if dependents:
        raise RequirementCompilationError(
            f"requirement graph node {node_id} is still referenced by: " + ", ".join(dependents)
        )
...
def compile_requirement_change(
    ...
    *,
    ...
    if mode == "add":
        ...
        graph, requirement_changed = _delete_requirement_node(graph, updated.requirement_id)
        records = [
            record
            for record in loaded.document.records
            if record.requirement_id != updated.requirement_id
    ...
    try:
        rationale = RationaleDocument.model_validate(read_json(rationale_path))
    except (OSError, json.JSONDecodeError, TypeError, ValueError) as exc:
    ...
    changed_node_ids = tuple(sorted(set(gpio_changed + label_changed + requirement_changed)))
    graph_content = (
