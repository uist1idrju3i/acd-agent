...
from acd.core.knowledge.design_predicates import evaluate_design_predicates, evaluate_strapping_pin
...
PLACEMENT_SCRIPT = "plugins/acd/skills/acd-placement-search/scripts/placement_search.py"
...
def _validate_dimensions(
    ...
    if disabled:
        raise ExplorationError(
            "candidate references non-explorable change dimensions: " + ", ".join(disabled)
        )
    unsupported = sorted(set(dimensions) - SEARCHABLE_DIMENSIONS)
    if unsupported:
        raise ExplorationError(
            "candidate references unsupported change dimensions: " + ", ".join(unsupported)
        )
...
def _placement_candidates(
    ...
    *,
    ...
    from acd.core.manufacturing.fab import load_fab_profile_registry, resolve_fab_profile_path
...
def _gpio_candidate_graph(
    ...
    nodes = [
        item.model_copy(
            update={
                "depends_on": ([u1.node_id] if item.id == old_pin.node_id else [u1.node_id, net])
            }
        )
    ...
    for node in sorted(
        ...
        for target_gpio in sorted(gpio_pads):
            if target_gpio == current:
                continue
            updated, change = _gpio_candidate_graph(graph, lane, node.id, target_gpio, gpio_pads)
            if not change:
                continue
            ...
            candidates.append(
                ExplorationCandidate(
                    ...
                    provenance={
                        ...
                        "script_name": "src/acd/core/knowledge/exploration.py",
                        "script_sha256": _sha256(
                            repository_root() / "src/acd/core/knowledge/exploration.py"
                        ),
                        "graph_revision": graph.revision,
                        "pass_evidence": False,
                    },
...
def _validated_evidence_payload(path: Path, target_revision: str | None) -> dict[str, Any]:
...
def load_remediation_requests(
    ...
    if not evidence_path.is_file():
        raise ExplorationError(f"structured gate evidence is missing: {evidence_path}")
    payload = _validated_evidence_payload(evidence_path, target_revision)
    if payload.get("gate") != "design_predicates":
        raise ExplorationError("structured gate evidence gate is not design_predicates")
    ...
    for item in cast(list[object], predicates):
        ...
        if not isinstance(dimensions, list) or not all(
            ...
            raise ExplorationError("structured gate evidence remediation dimensions are malformed")
        ...
        requests.append(
            RemediationRequest(
                ...
                change_dimensions=tuple(sorted(cast(list[str], dimensions))),
...
def _decoupling_placement_candidates(
    ...
    subjects = {
        ...
        if request.refdes is not None and "component_placement_xy" in request.change_dimensions
    }
    if not subjects:
        return ()
    try:
        ...
        raise ExplorationError(f"decoupling remediation could not be solved: {exc}") from exc
    placements = {
        item.refdes: item for item in report.placements if item.changed and item.refdes in subjects
    }
    if not placements:
        return ()
    return (
        ExplorationCandidate(
            ...
            provenance={
                ...
                "script_name": "src/acd/core/electrical/decoupling_placement.py",
                "script_sha256": _sha256(
                    repository_root() / "src/acd/core/electrical/decoupling_placement.py"
                ),
                "graph_revision": graph.revision,
...
def _remediation_dimensions(
    ...
    return tuple(
        sorted(
            {dimension for request in remediation for dimension in request.change_dimensions}
...
def _remediation_candidates(
    ...
    *,
    ...
    if "component_placement_xy" in dimensions:
        candidates.extend(_decoupling_placement_candidates(graph, fixture_dir, remediation))
        candidates.extend(
...
def _refresh_working_rationale(
    ...
    try:
        ...
        raise ExplorationError(f"candidate rationale could not be refreshed: {exc}") from exc
...
def _candidate_outcome(
    ...
    failed = tuple(
        result for result in pre_router if result.status not in {"pass", "not_applicable"}
    )
    if failed:
        dimensions = tuple(
            sorted(
        ...
        while pending:
            ...
            if survived:
                if dry_run:
                    ...
                    break
                try:
                    commit = _commit_candidate(working_fixture, source_graph, source_fixture)
                except ExplorationError as exc:
