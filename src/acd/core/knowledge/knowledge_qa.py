...
def _nodes_of_kind(graph: DesignGraph, kind: str) -> tuple[GraphNode, ...]:
    return tuple(sorted((node for node in graph.nodes if node.kind == kind), key=lambda n: n.id))
...
def _troubleshooting_answer(base: KnowledgeBase, question: str) -> KnowledgeAnswer:
    ...
    if unknown_entries:
        reasons = "; ".join(f"{entry.entry_id}: {entry.reason}" for entry in unknown_entries)
        return _unknown(
            base,
            question,
            "troubleshooting",
...
def _rationale_citation(base: KnowledgeBase, record: RationaleRecord) -> KnowledgeCitation:
    sources = base.index.available("rationale")
    reference = sources[0].reference if sources else "rationale"
    return KnowledgeCitation(kind="rationale", reference=reference, locator=record.rationale_id)
