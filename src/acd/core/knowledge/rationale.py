...
REQUIRED_RATIONALE_ATTRS: Final[dict[str, frozenset[str]]] = {
    ...
    "safety.redundant_group": frozenset({"members", "resources_shared_forbidden"}),
...
RATIONALE_EXEMPT_ATTRS: Final[dict[str, dict[str, str]]] = {
    ...
    "electrical.component": {
        ...
        "power_input_net": ("Power input net identifies the declared power-tree connectivity."),
        "power_output_net": ("Power output net identifies the declared power-tree connectivity."),
    },
    "electrical.net": {
        ...
        "routing_layer": ("Routing layer identifies the declared conductor geometry source."),
    },
    "fab.order_intent": {
    ...
    "mechanical.silk_text": {
        ...
        "label": ("Silkscreen label is a human-readable connector identification declaration."),
    },
    "requirement": {
...
def summarize_rationale_coverage(report: RationaleCoverageReport, *, limit: int = 5) -> str:
    ...
    return " ".join(
        [
            ...
            _group(
                "stale",
                [f"{item.rationale_id}→{_subject_text(item.subject)}" for item in report.stale],
            ),
            _group(
                "unknown_provenance",
            ...
            _group(
                "unclassified",
                [f"{item.node_id}.{item.attr} ({item.reason})" for item in report.unclassified],
            ),
            _group(
                "templated",
            ...
            _group(
                ...
                [f"{item.rationale_id} ({item.reason})" for item in report.generator_violations],
            ),
        ]
    )
