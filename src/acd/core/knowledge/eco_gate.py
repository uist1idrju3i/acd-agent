...
from acd.core.manufacturing.defect_records import check_defect_records
...
def _lanes_for_kind(kind: str) -> frozenset[str]:
    ...
    if kind.startswith(("safety.", "fab.", "design.")) or kind in {
        "requirement",
        "safety",
        "fab",
        "design",
...
def _impact_status(
    ...
    if missing_lanes:
        matched = False
        reasons.append("impact lanes do not cover graph changes: " + ", ".join(missing_lanes))
...
def _gate_runs(
    ...
    required = {gate for lane in derived_lanes for gate in MINIMUM_GATES[lane]}
...
def _horizontal_status(
    ...
    target_ids = {reason.ref for reason in record.reasons if reason.kind == "defect"}
    if not target_ids:
        if record.horizontal_dispositions:
            reasons.append("horizontal dispositions require a referenced defect reason")
            return "incomplete"
        return "complete"
    by_id = {item.defect_id: item for item in defects.records}
    missing_defects = sorted(target_ids - set(by_id))
    if missing_defects:
        reasons.append("ECO references unknown defects: " + ", ".join(missing_defects))
        return "incomplete"
    defect_check = check_defect_records(from_graph, defects)
    target_findings = [
        finding for finding in defect_check.findings if finding.defect_id in target_ids
    ]
    ...
    dispositions = {(item.defect_id, item.node_id): item for item in record.horizontal_dispositions}
    ...
    for defect_id in sorted(target_ids):
        ...
        for scope in defect.horizontal_scopes:
            if scope.search_status == "searched":
                expected.update((defect_id, node_id) for node_id in scope.matched_node_ids)
    ...
    for key, disposition in dispositions.items():
        ...
        if disposition.disposition == "fixed_in_eco" and disposition.node_id not in impact_nodes:
            reasons.append(f"fixed_in_eco node is outside ECO impact: {disposition.node_id}")
            status = "incomplete"
        if disposition.disposition == "deferred" and disposition.deferred_to not in eco_ids:
            reasons.append(f"deferred ECO is not in the document: {disposition.deferred_to}")
            status = "incomplete"
    return status
...
def evaluate_eco(
    *,
    ...
    impact_nodes = frozenset(
        record.impact.nodes_added + record.impact.nodes_removed + record.impact.nodes_changed
    )
