...
def load_dialogue_history(path: Path) -> IdeaDialogueHistory:
    ...
    try:
        return IdeaDialogueHistory.model_validate_json(
            path.read_text(encoding="utf-8")
        )
    except (OSError, ValueError) as exc:
        raise IdeaDialogueError(
            f"dialogue history {path} is not valid: {exc}"
        ) from exc
...
def _set_field(record: IdeaRecord, path: str, field: IdeaField) -> IdeaRecord:
    ...
    if head == "success_criteria":
        updated = [
            item.model_copy(update={"statement": field})
            if item.criterion_id == tail
            else item
            for item in record.success_criteria
        ]
        return record.model_copy(update={"success_criteria": updated})
    updated = [
        item.model_copy(update={"description": field})
        if item.function_id == tail
        else item
        for item in record.functions
    ]
...
def apply_turn(
    ...
    if history.idea_id != record.idea_id:
        raise IdeaDialogueError(
            f"history idea_id {history.idea_id!r} does not match "
            f"record idea_id {record.idea_id!r}"
        )
    expected_turn = len(history.turns) + 1
    if turn.turn_no != expected_turn:
        raise IdeaDialogueError(
            f"turn_no {turn.turn_no} is not the next turn {expected_turn}"
        )
    seen: set[str] = set()
    updated = record
    for question in turn.asked:
        if not _valid_path(question.field):
            raise IdeaDialogueError(
                f"question field {question.field!r} is not an idea field path"
            )
    for answer in turn.answers:
                ...
                update={
                    "success_criteria": [
                        *updated.success_criteria,
                        IdeaSuccessCriterion(
                            criterion_id=item_id, statement=confirmed
                        ),
                    ]
                }
            )
        ...
        if field is None:
            raise IdeaDialogueError(
                f"answer field {path!r} is not an idea field path"
            )
        if field.status == "confirmed":
            raise IdeaDialogueError(
                f"field {path!r} is already confirmed; re-deciding requires "
                "a new idea revision"
            )
        updated = _set_field(updated, path, confirmed)
    new_history = history.model_copy(
        update={"turns": [*history.turns, turn]}
    )
    return updated, new_history


def progress_summary(
    record: IdeaRecord, history: IdeaDialogueHistory
) -> IdeaProgress:
