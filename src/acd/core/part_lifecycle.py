...
def evaluate_part_lifecycle(
    ...
    governed_entries = {
        mpn: entries[mpn] for mpn in rows_by_mpn if mpn and mpn in entries
    }
    for mpn, entry in governed_entries.items():
        ...
        if age_days < 0 or age_days > registry.policy.max_status_age_days or (
    ...
    for mpn, rows_for_mpn in rows_by_mpn.items():
        ...
        entry = entries[mpn]
        valid = all(
            _alternate_for_footprint(entry, candidate.footprint)
            for candidate in rows_for_mpn
        )
