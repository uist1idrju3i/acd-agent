...
def _read_capability_input(
    ...
    raise FirmwareCapabilityContractError(
        f"firmware capability declaration is neither a JSON object nor a readable file: {value}"
    )
...
def _validate_new_capability(
    ...
    if capability.capability_id in {item.capability_id for item in registry.capabilities}:
        raise FirmwareCapabilityContractError(
            f"firmware capability_id is already registered: {capability.capability_id!r}"
        )
    declared_actions = {action for item in registry.capabilities for action in item.actions}
    conflicting = sorted(declared_actions & set(capability.actions))
    if conflicting:
        raise FirmwareCapabilityContractError(
            "firmware capability actions are already registered: " + ", ".join(conflicting)
        )
    ...
    if unknown_roles:
        raise FirmwareCapabilityContractError(
            "firmware capability requires undeclared pin roles: " + ", ".join(unknown_roles)
        )
...
def register_firmware_capability(
    ...
    *,
    ...
    registry = load_firmware_capability_registry(registry_path or _default_registry_path())
