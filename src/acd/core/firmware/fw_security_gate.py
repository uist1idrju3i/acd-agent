    ...
    *,
    ...
    try:
        ...
        expected_flags = {
            ...
            "CONFIG_SECURE_BOOT_V2_ENABLED": declaration.secure_boot.scheme == "secure_boot_v2",
        ...
        for key, expected in expected_flags.items():
            actual = (
                config.get(key) == "y"
                if key != "CONFIG_BOOTLOADER_APP_SECURE_VERSION"
                else (config.get(key) == str(declaration.ota.secure_version))
            )
            if actual != expected:
                findings.append(
                    _finding(
        ...
        if flash_size is not None:
                ...
                int(_text_field(item, "offset_hex"), 16) + int(_text_field(item, "size_hex"), 16)
    ...
    status = (
        ...
        else ("unknown" if findings else "pass")
    )
    return FirmwareSecurityGateResult(
        ...
        + hashlib.sha256(declaration.model_dump_json(by_alias=False).encode("utf-8")).hexdigest(),
        findings=findings,
    )
...
def validate_security_gate_result(
    ...
    *,
    ...
    try:
        result = FirmwareSecurityGateResult.model_validate_json(path.read_text(encoding="utf-8"))
