...
from acd.core.firmware.firmware_coverage import FirmwareCoverageCode, FirmwareCoverageFinding
...
def load_firmware_coverage_findings(
    ...
    try:
        ...
        raise ExplorationError(f"firmware coverage report is unreadable: {path}: {exc}") from exc
    ...
    for item in cast(list[object], findings):
        ...
        if (
            ...
            raise ExplorationError("firmware coverage finding is malformed (fail-closed)")
        result.append(
...
def explore_firmware_candidates(
    ...
    *,
    ...
    requested = {dimension for request in remediation for dimension in request.change_dimensions}
