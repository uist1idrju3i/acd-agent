...
def _require_dependency_set(node: GraphNode, expected: set[str], relation: str) -> None:
...
def _require_kind(
    ...
    if target is None:
        raise GraphExtractionError(f"node reference {field}={node_id!r} does not exist")
    if target.kind != expected_kind:
        raise GraphExtractionError(f"node reference {field}={node_id!r} is not {expected_kind}")
    return target
...
def extract_firmware_lane(graph: DesignGraph) -> FirmwareLane:
    ...
    if initial_states[0].node_id != entry_state:
        raise GraphExtractionError("firmware.module entry_state does not match the initial state")

    transition_nodes = sorted(
        (node for node in graph.nodes if node.kind == "firmware.state_transition"),
        key=lambda node: node.id,
    )
    if not transition_nodes:
        raise GraphExtractionError("firmware lane requires at least one state transition")
    ...
    for node in transition_nodes:
        ...
        if key in transition_keys:
            raise GraphExtractionError(f"duplicate firmware state transition declaration: {key!r}")
    ...
    if transition_states != state_ids:
        ...
        raise GraphExtractionError(
            "firmware states do not all participate in transitions: " + ", ".join(missing)
        )
    reachable = {entry_state}
    while True:
        expanded = reachable | {
            transition.to_state for transition in transitions if transition.from_state in reachable
        }
    ...
    if not sequence_nodes:
        raise GraphExtractionError("firmware lane requires at least one sequence step")
    ...
    for node in sequence_nodes:
        ...
        if step_index in sequence_ids:
            raise GraphExtractionError(f"duplicate firmware sequence step index: {step_index}")
        ...
        if target_node is None:
            raise GraphExtractionError(f"node reference target={target!r} does not exist")
        if target_node.kind not in {"firmware.module", "electrical.component"}:
            raise GraphExtractionError(f"node reference target={target!r} has an unsupported kind")
