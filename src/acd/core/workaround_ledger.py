    ...
    lots = sorted(
        {
            ...
            for lot in record.affected_units.lots
        }
    )
    serials = sorted(
        {
            serial
            for record in records
            for serial in record.affected_units.serials
        }
    )
    return (
        [UnitRef(lot=lot) for lot in lots]
        + [UnitRef(serial=serial) for serial in serials],
        [],
    )
...
def evaluate_workaround_retirement(
    *,
    ...
    retirement = next(
        (
            item
            for item in ledger.retirements
            if item.workaround_id == workaround_id
        ),
        None,
    )
    ...
    for unit in affected:
        ...
        if application is not None and not valid_application:
            reasons.append(
                f"{_unit_label(unit)} application revisions do not match rework"
            )
        ...
        if disposition is not None:
            if disposition.disposition == "scrapped" or (
                ...
                and disposition.revision == retirement.resolved_revision
            ):
                closed_by_disposition = True
            else:
                reasons.append(
                    f"{_unit_label(unit)} upgraded_to_revision does not match "
                    "resolved revision"
                )
