...
from acd.core.mechanical.structural_safety import INCOMPATIBLE_SIGNAL_CLASS_PAIRS, SIGNAL_CLASSES
...
def _graph_pin_for_cavity(
    ...
    for pin in lane.pins_of_component(component_id):
        ...
        if pin.pad == cavity or (node is not None and node.attrs.get("name") == cavity):
...
def _netlist_consistency(
    ...
    connector_map = {
        connector.connector_id: connector.graph_component_id for connector in contract.connectors
    }
    failures: list[str] = []
    unknowns: list[str] = []
...
def _temperature_factor(
    ...
    for left, right in pairwise(points):
        if left.temp_c <= ambient_temperature_c <= right.temp_c:
            ratio = (ambient_temperature_c - left.temp_c) / (right.temp_c - left.temp_c)
...
def _ampacity(
    ...
    bundle_factor = contract.bundle_derating.factor if contract.bundle_derating is not None else 1.0
    for wire in contract.wires:
...
def _voltage_drop(
    ...
    for wire in contract.wires:
        ...
        if wire.return_wire_id is not None:
            ...
            return_resistance = return_type.resistance_mohm_per_m * _wire_length_m(return_wire)
...
def _shield_requirement(graph: DesignGraph, contract: HarnessContract) -> HarnessCheckResult:
    required = [
        wire
        for wire in contract.wires
    ...
    for wire in required:
        wire_type = _wire_type(contract, wire)
        paired = wires.get(wire.return_wire_id) if wire.return_wire_id is not None else None
...
def _signal_class_segregation(graph: DesignGraph, contract: HarnessContract) -> HarnessCheckResult:
    ...
    for left_id, right_id in sorted(pairs):
        ...
        if frozenset({left_class, right_class}) in INCOMPATIBLE_SIGNAL_CLASS_PAIRS:
            failures.append(f"{left_id}/{right_id}: {left_class}/{right_class}")
    if failures:
        return _check(
            "signal_class_segregation",
            "fail",
            "incompatible signal classes share a connector or harness route",
            sorted({wire_id for pair in failures for wire_id in pair.split(":", 1)[0].split("/")}),
            {"pairs": failures},
        )
    if unknowns:
        return _check(
...
def _member_net_ids(graph: DesignGraph, lane: ElectricalLane, member_id: str) -> set[str] | None:
...
def _redundant_group_harness(
    ...
    groups = [
        ...
        and "harness" in cast(list[object], node.attrs["resources_shared_forbidden"])
    ]
    if not groups:
        return _check(
    ...
    for group in groups:
        ...
        if len(member_nets) != len(members):
            continue
        if any(
            all(nets & route_nets_item for nets in member_nets) for route_nets_item in route_nets
        ):
