...
from acd.core.electrical.sexpr import SExpr, parse_one
...
def _child(node: SExpr, tag: str) -> list[SExpr] | None:
    if not isinstance(node, list):
        return None
    return next(
        (child for child in node[1:] if isinstance(child, list) and child and str(child[0]) == tag),
        None,
    )
...
def _path_for(
    ...
    for via in relevant_vias:
        via_nodes = [_node_for(nodes, layer, via.point) for layer in via.layers]
    ...
    source_nodes = {
        ...
        if any(
            math.dist(node[1:], pad.point) <= SNAP_TOLERANCE_MM and node[0] == layer
            for node in nodes
        )
    }
    sink_nodes = {
    ...
    queue: list[tuple[float, tuple[str, float, float], int, tuple[_Edge, ...]]] = [
        (0.0, node, index, ()) for index, node in enumerate(sorted(source_nodes))
    ]
    sequence = len(queue)
    distances: dict[tuple[str, float, float], float] = {node: 0.0 for node in source_nodes}
    ...
    if worst_density is not None and worst_density > path.max_current_density_a_per_mm2:
        status = "fail"
