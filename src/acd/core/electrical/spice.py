...
def _net_names(lane: ElectricalLane) -> dict[str, str]:
    return {net.node_id: net.name for net in lane.nets}


def _component_map(lane: ElectricalLane) -> dict[str, ComponentView]:
    return {component.refdes: component for component in lane.components}
...
def _pins_as_nodes(
    ...
    return tuple(
        _node_name(names[net_id]) for net_id in _component_pins(lane, component) if net_id in names
    )
...
def _check_revision(graph: DesignGraph, request: SpiceAnalysisRequest) -> None:
    if request.graph_id != graph.graph_id or request.revision != graph.revision:
        raise SpiceNetlistError("graph/request graph_id or revision mismatch")
...
def extract_power_netlist(
    ...
    for entry in request.models.decoupling:
        ...
        if not component.refdes.upper().startswith("C"):
            raise SpiceNetlistError(f"decoupling component {entry.refdes!r} is not a capacitor")
    ...
    if request.sources.vbus_ramp_ms > 0:
        lines.append(f"* transient ramp requested: {request.sources.vbus_ramp_ms:.12g} ms")
    ...
    for component in sorted(components.values(), key=lambda item: item.refdes):
        ...
        if refdes.upper().startswith("R") or refdes.upper().startswith("C"):
            ...
            lines.append(f"{suffix}_{refdes} {pins[0]} {pins[1]} {value:.12g}")
    ...
    if len(ldo_pins) < 3:
        ...
        if request.models.ldo.iq_a > 0:
            lines.append(
                f"IQ_{ldo.refdes} {_node_name(output_name)} 0 {request.models.ldo.iq_a:.12g}"
            )
    ...
    for entry in request.models.led:
        ...
        lines.append(f".model {model_name} D(Is={saturation_current:.12g} N=1.8)")
        if entry.drive is None:
            ...
            if len(shared) != 1 or len(drive_nets) != 1:
                findings.append(f"{entry.refdes}: cannot resolve series resistor drive net")
            else:
                drive_node = _node_name(names[next(iter(drive_nets))])
                ...
                lines.append(f"B_LED_{entry.refdes} {drive_node} 0 V={source}")

    for index, entry in enumerate(sorted(request.models.i2c_pullups, key=lambda item: item.net)):
        ...
        if tran_for_stimulus is not None:
            ...
            lines.append(f"S_OD_{index + 1} {node} 0 ctrl_{index + 1} 0 SW_OD")
    if request.models.i2c_pullups:
        lines.append(".model SW_OD SW(Ron=1e-3 Roff=1e9 Vt=1 Vh=0.1)")
    ...
    print_targets = sorted(
        {
            ...
            if limit.quantity == "node_voltage" and limit.target in set(names) | set(names.values())
        }
        | {
            f"V({_node_name(_resolve_net(limit.target, names))})"
            for limit in request.limits
            if limit.quantity == "rise_time" and limit.target in set(names) | set(names.values())
        }
        | {
...
def _parse_output(
    ...
    for line in text.splitlines():
        ...
        if not tokens:
            continue
        if any(token.lower().startswith(("v(", "i(", "@")) for token in tokens):
            headers = []
            for token in tokens:
    ...
    return (
        measures,
        {key: tuple(value) for key, value in sorted(traces.items())},
...
def run_ngspice(netlist: SpiceNetlist, workdir: Path) -> SpiceRawResult:
    ...
    if version != netlist.version_pin:
        return SpiceRawResult(
            ...
            raw_output_sha256="sha256:" + hashlib.sha256(version_text.encode("utf-8")).hexdigest(),
            measures={},
            traces={},
...
def evaluate_spice(
    ...
    for limit in request.limits:
        ...
        if _has_degenerate_measurement(raw, limit, measured, netlist.node_map):
        ...
        passed = (lower is None or measured >= lower) and (upper is None or measured <= upper)
        checks.append(
            SpiceCheck(
                quantity=limit.quantity,
                target=limit.target,
