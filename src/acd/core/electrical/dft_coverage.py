...
def _classify_nets(graph: DesignGraph, lane: ElectricalLane) -> dict[str, set[str]]:
    ...
    for component in lane.components:
        for pin in lane.pins_of_component(component.node_id):
            ...
            if pin.net_id is not None and ("sda" in function or "scl" in function):
                i2c.add(pin.net_id)
    assignments = tuple(node for node in graph.nodes if node.kind == "firmware.pin_assignment")
    ...
    usb = {
        net.node_id
        for net in lane.nets
        if net.name.upper() in _USB_NAMES or ("usb" in (net.differential_pair or "").lower())
    }
    non_power_ground = power | ground
    all_signals = {net.node_id for net in lane.nets if net.node_id not in non_power_ground}
    return {
        "power_rail": power,
        "ground": ground,
        "i2c": i2c,
...
def _body_rectangles(
    ...
    for body in graph.nodes:
        ...
        rotation = rotation if rotation is not None else _number(body.attrs.get("rotation_deg"))
...
def evaluate_dft_coverage(
    ...
    no_pin_nets = sorted(
        net_id for net_id in required_nets if not any(pin.net_id == net_id for pin in lane.pins)
    )
    ...
    coverage_details = {
        ...
        "net_names": {net.node_id: net.name for net in lane.nets if net.node_id in required},
    }
    if no_pin_nets:
        ...
        coverage_check = _check(
            "net_coverage",
            "unknown",
            "required net classes could not be classified: " + ", ".join(unresolved_classes),
    ...
    if missing_centers:
        ...
        for (left_id, left), (right_id, right) in combinations(sorted(centers.items()), 2):
    ...
    diameter_sources = {component_id: source for component_id, (_, source) in diameter_info.items()}
