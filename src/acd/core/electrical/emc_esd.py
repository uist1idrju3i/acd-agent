...
def _external_signal_nets(lane: ElectricalLane, component: ComponentView) -> tuple[str, ...]:
    return tuple(
        ...
        if _net_name(lane, net_id) != "GND" and not lane.net_by_id(net_id).power_rail
    )
        ...
        for capacitor, value in capacitors:
            ...
            try:
                ...
                if source_center is not None and capacitor_center is not None:
                    area = math.dist(source_center, capacitor_center) * lane.board.thickness_mm
                    rail_details.append(
                        {
...
def _return_path_result(lane: ElectricalLane, environment: UseEnvironment) -> EmcEsdPredicateResult:
