...
def _points_attr(node: GraphNode) -> tuple[tuple[float, float], ...]:
    ...
    for item in value:
        ...
        try:
            ...
            raise GraphExtractionError(f"node {node.id!r}: polygon point is not numeric") from exc
...
def _graphic_parts_attr(
    ...
    for encoded in value:
        try:
            ...
            raise GraphExtractionError(f"node {node.id!r}: graphic part is malformed") from exc
        ...
        for raw_contour_value in raw_contours:
            if not isinstance(raw_contour_value, list):
                raise GraphExtractionError(f"node {node.id!r}: graphic contour is invalid")
            ...
            for point_value in raw_contour:
                if not isinstance(point_value, list):
                    raise GraphExtractionError(f"node {node.id!r}: graphic point is invalid")
...
def _rotation_degrees_attr(node: GraphNode) -> tuple[float, ...]:
    ...
    if value is None:
        return (0.0, 90.0)
    if not isinstance(value, list) or not value:
        raise GraphExtractionError(f"node {node.id!r}: placement_rotation_degrees is invalid")
    try:
...
def _placement_center_attr(node: GraphNode) -> tuple[float, float] | None:
    ...
    if not isinstance(value, str):
        raise GraphExtractionError(f"node {node.id!r}: placement_center_mm is invalid")
    parts = value.split(",")
    if len(parts) != 2:
        raise GraphExtractionError(f"node {node.id!r}: placement_center_mm is invalid")
    try:
        ...
        raise GraphExtractionError(f"node {node.id!r}: placement_center_mm is invalid") from exc
        ...
        if node.kind == "mechanical.silk_text":
            ...
            texts.append(
                SilkTextView(
                    ...
                    x_mm=(
                        _number_attr(node, "x_mm") if node.attrs.get("x_mm") is not None else None
                    ),
                    y_mm=(
                        _number_attr(node, "y_mm") if node.attrs.get("y_mm") is not None else None
                    ),
                    layer=layer,
                    height_mm=height,
                    ...
                    placement_offset_step_mm=_number_attr(node, "placement_offset_step_mm"),
                    placement_search_limit_mm=_number_attr(node, "placement_search_limit_mm"),
                    board_edge_margin_mm=_number_attr(node, "board_edge_margin_mm"),
                    board_edge_margin_source=_str_attr(node, "board_edge_margin_source"),
                    placement_rotation_degrees=_rotation_degrees_attr(node),
                    placement_safety_margin_mm=_number_attr(node, "placement_safety_margin_mm"),
                )
            )
            ...
            stroke = _number_attr(node, "stroke_width_mm")
            if stroke < 0:
                raise GraphExtractionError(f"node {node.id!r}: stroke width must be nonnegative")
            ...
            graphics.append(
                SilkGraphicView(
                    ...
                    board_edge_margin_mm=_number_attr(node, "board_edge_margin_mm"),
                    board_edge_margin_source=_str_attr(node, "board_edge_margin_source"),
                    contours=tuple(contour for part in parts for contour in part.contours),
                    fill=parts[0].fill,
                    fill_rule=parts[0].fill_rule,
                    parts=parts,
                    source_path=(source_path_attr if isinstance(source_path_attr, str) else None),
                    source_sha256=(
                        source_sha256_attr if isinstance(source_sha256_attr, str) else None
                    ),
