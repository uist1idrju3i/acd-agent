...
def load_placement_coupling_constraints(
    ...
    for node in graph.nodes:
        ...
        if not isinstance(refdes, str) or not refdes:
            raise PlacementConstraintError(f"component {node.id!r} has malformed refdes")
    ...
    for node in sorted(
        ...
        dependency_ids = {
            dependency for dependency in node.depends_on if dependency in set(components.values())
        }
        ...
        if not isfinite(max_distance):
            raise PlacementConstraintError(f"group {node.id!r} max_distance_mm must be finite")
