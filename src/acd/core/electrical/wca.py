...
def _nominal(
    ...
    check: SpiceCheck | None = next(
        (item for item in spice_result.checks if item.target == target and item.status == "pass"),
        None,
    )
...
def _quantity_result(
    ...
    if quantity.kind == "power_budget_peak":
        nominal, power_bias, power_random, power_findings = _power_budget(quantity, power_budget)
    ...
    random_rss = math.sqrt(sum(component.value_pct**2 for component in random))
...
def evaluate_wca(
    ...
    if environment is not None and (
        environment.graph_id != graph.graph_id or environment.revision != graph.revision
    ):
        raise WcaInputError("graph/environment graph_id or revision mismatch")
    if spice_result is not None and (
        spice_result.graph_id != graph.graph_id or spice_result.revision != graph.revision
    ):
    ...
    findings = [
        f"{result.quantity_id}: {finding}" for result in results for finding in result.findings
    ]
    return WcaResult(
