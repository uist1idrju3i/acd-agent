...
_NORMALIZED_TITLE = "<title>SVG Image created as normalized.svg date 1970-01-01T00:00:00Z </title>"
_TITLE_CONTENT = (
...
_ATTRIBUTE_PATTERN = re.compile(r'(?P<name>width|height|viewBox)\s*=\s*"(?P<value>[^"]*)"')
    ...
    for _position, is_open, match in events:
        if is_open:
            ...
            attributes = {
                ...
                for item in _RAW_SVG_ATTRIBUTE_PATTERN.finditer(opening.group("attributes"))
            }
...
def normalize_kicad_layer_svg(svg: bytes) -> bytes:
    ...
    if text.count("<title") != 2 or text.count("</title>") != 2:
        raise SvgNormalizationError("KiCad layer SVG must contain exactly two title elements")
    ...
    if not bounds[0] < title.start() < bounds[1]:
        raise SvgNormalizationError("KiCad layer SVG title must be inside svg#layer-view")
    return (text[: title.start()] + _NORMALIZED_TITLE + text[title.end() :]).encode("utf-8")
...
def svg_source_hash(svg: bytes, normalization_rule_id: str) -> str:
    ...
    raise SvgNormalizationError(f"unsupported SVG normalization rule: {normalization_rule_id}")
