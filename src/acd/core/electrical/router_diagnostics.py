...
_CONVERGENCE_STATES = frozenset({"converged", "not_converged", "timed_out", "unknown"})
...
def _progression(payload: dict[str, object], errors: list[str]) -> tuple[int, ...]:
    ...
    for index, value in enumerate(cast(list[object], unrouted)):
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            errors.append(
                f"{_ROUTER_PROGRESS_PATH.name}: 'unrouted[{index}]' is not a non-negative integer"
            )
            return ()
...
def _convergence_state(payload: dict[str, object], errors: list[str]) -> str:
    ...
    if not isinstance(state, str) or state not in _CONVERGENCE_STATES:
        errors.append(f"{_ROUTER_PROGRESS_PATH.name}: 'convergence_state' is missing or invalid")
        return "unknown"
    return state


def _open_nets(payload: object | None, errors: list[str]) -> tuple[int | None, tuple[str, ...]]:
    if payload is None:
        return None, ()
    ...
    nets = (
        cast(dict[str, object], observation).get("nets") if isinstance(observation, dict) else None
    )
    if not isinstance(nets, list):
        errors.append(f"{_CONNECTIVITY_PATH.name}: 'observation.nets' is missing or not a list")
        return None, ()
    open_names: list[str] = []
    for index, entry in enumerate(cast(list[object], nets)):
        if not isinstance(entry, dict):
            errors.append(
                f"{_CONNECTIVITY_PATH.name}: 'observation.nets[{index}]' is not an object"
            )
            return None, ()
...
def router_diagnostics_hint(diag: RouterDiagnostics | None) -> str | None:
    ...
    if diag.convergence_state == "timed_out":
        return "router timed out; raise --router-timeout-s or reduce routing load"
