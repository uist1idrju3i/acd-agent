...
CHECKER_NAME = "acd.core.electrical.projection_format_check"
CHECKER_VERSION = "1"
...
def _balanced_parentheses(path: Path, value: str, *, track_quotes: bool = True) -> int:
    depth = 0
    maximum = 0
    quoted = False
    escaped = False
...
def _check_excellon(path: Path, data: bytes) -> dict[str, object]:
    ...
    try:
        header_end = next(
            index for index, line in enumerate(lines[1:], start=1) if line in {"%", "M95"}
        )
    except StopIteration as exc:
        raise ProjectionFormatError(path, "Excellon header terminator is missing") from exc
    tools = sum(1 for line in lines[1:header_end] if re.match(r"^T\d+C", line) is not None)
    if lines[-1] != "M30":
...
def _check_stl(path: Path, data: bytes) -> dict[str, object]:
    ...
    if (
        ...
        return {
            "encoding": "ascii",
            "triangle_count": sum(
                1 for line in value.splitlines() if line.strip().startswith("facet normal")
            ),
        }
