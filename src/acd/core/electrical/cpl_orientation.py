...
def cpl_orientation_attrs(
    ...
    values = {
        ...
        **{key.removeprefix("cpl_rotation_"): value for key, value in evidence_attrs.items()},
    }
    unexpected = set(values) - CPL_ORIENTATION_ATTR_KEYS
    if unexpected:
        raise ValueError("unsupported CPL orientation attributes: " + ", ".join(sorted(unexpected)))
    ...
    if isinstance(source, str):
        try:
            ...
            raise ValueError(f"{refdes}: malformed CPL geometry evidence source") from exc
    return cast(
        dict[str, AttrValue],
