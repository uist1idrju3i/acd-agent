...
from acd.core.electrical.electrical import ElectricalLane, GraphExtractionError
...
def derive_net_widths(
    ...
    if formula_type != "ipc2221_external_current_capacity":
        raise GraphExtractionError("unsupported IPC-2221 width basis equation (fail-closed)")
    if not source:
    ...
    for net in sorted(lane.nets, key=lambda item: item.name):
        ...
        if net.width_basis == "current_ipc2221":
            ...
            area_mil2 = (current / (external_k * temperature_rise**external_b)) ** (
                1.0 / external_c
            )
            ...
            if net.manufacturing_margin_mm is None:
                raise GraphExtractionError(f"net {net.name!r}: manufacturing margin is required")
            margin = net.manufacturing_margin_mm
            if not math.isfinite(margin) or margin < 0:
                raise GraphExtractionError(f"net {net.name!r}: manufacturing margin is invalid")
            current = None
...
def group_netclasses(
    ...
    for requirement in requirements:
        groups.setdefault(round(requirement.adopted_width_mm, 6), []).append(requirement.net_name)
    return tuple(
