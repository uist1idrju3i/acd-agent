...
def _apply_remove(
    ...
    removed = {
        ...
        or (
            node.kind == "electrical.pin"
            and node.attrs.get("component") == operation.component_id
        )
        or (
            node.kind == "mechanical.component_body"
            and operation.component_id in node.depends_on
        )
    }
    remaining = {
        node.id: node
    ...
    dangling = sorted(
        ...
        if set(node.depends_on) & removed
    )
    if dangling:
        raise ReworkDiffError(
            "removing component would leave dangling dependencies: "
            + ", ".join(dangling)
        )
    nodes.clear()
...
def _apply_mechanical(
    ...
    for key, value in operation.attrs.items():
        ...
        if key not in node.attrs or not _is_number(current) or not _is_number(value):
            raise ReworkDiffError(
                f"mechanical attribute {key!r} must already exist as numeric"
            )
    attrs = dict(node.attrs)
    attrs.update(operation.attrs)
...
def apply_rework_diff(
    ...
    *,
    ...
    nodes = _node_map(
        [
            GraphNode.model_validate(node.model_dump(mode="json"))
