...
_TRANSFORM_FUNCTION_PATTERN = re.compile(
    ...
    def visit(element: ElementTree.Element, ancestors: tuple[ElementTree.Element, ...]) -> None:
        ...
        if tag in {"text", "tspan"}:
            ...
            if content.strip():
                ...
                x = _parse_number(
                    _inherited_attribute(element, ancestors, "x"), default=0.0
                )
                y = _parse_number(
                    _inherited_attribute(element, ancestors, "y"), default=0.0
                )
                (
                    offset_x,
                    offset_y,
                ...
                if not transform_resolved:
                    findings.append(
                        ReadabilityFinding(
                            ...
                            element_id=element_id,
                            detail=(
                                "supported transforms are translate, scale, and "
                                "diagonal matrix"
                            ),
                        )
                    )
    ...
    for box in boxes:
        ...
        for ancestor in reversed(box.ancestors):
            if _local_name(ancestor.tag) == "rect":
                background_value = ancestor.attrib.get("fill") or _style_attributes(
                    ancestor
                ).get("fill")
        ...
        if background_value is None and box.ancestors:
            ...
            for sibling in root_element:
                if _local_name(sibling.tag) != "rect":
                    continue
                background_value = sibling.attrib.get("fill") or _style_attributes(
                    sibling
                ).get("fill")
                if background_value is not None:
                    break
        if background_value is None:
            background_value = root.attrib.get("background") or _style_attributes(
                root
