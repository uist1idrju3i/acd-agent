...
def load_lane_artifact_retention(
    ...
    try:
        ...
        raise LaneArtifactRetentionError(
            f"lane artifact retention contract is invalid or unreadable: {resolved}: {exc}"
        ) from exc
...
def _file_sha256(path: Path) -> str:
    try:
        ...
        raise LaneArtifactRetentionError(f"cannot hash retained artifact: {path}: {exc}") from exc
