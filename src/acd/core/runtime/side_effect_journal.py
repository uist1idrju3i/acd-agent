...
def _parse_entry(line: str, line_number: int) -> JournalEntry:
    try:
        ...
        raise SideEffectJournalError(f"journal line {line_number} is not valid JSON") from exc
    ...
    raise SideEffectJournalError(f"journal line {line_number} has an unsupported entry type")
...
def _validate_entries(entries: Sequence[JournalEntry], *, require_complete: bool) -> None:
    ...
    for index, entry in enumerate(entries, start=1):
        if previous is None:
            ...
            raise SideEffectJournalError(f"journal hash chain is broken at line {index}")
        if previous is not None and entry.occurred_at < previous.occurred_at:
            raise SideEffectJournalError(f"journal timestamp moved backwards at line {index}")
    ...
    if require_complete:
        ...
        if missing_results:
            raise SideEffectJournalError("journal has pre-order entries without post-order results")
...
def read_journal(
    ...
    *,
    ...
    entries = tuple(
        _parse_entry(line, line_number) for line_number, line in enumerate(lines, start=1)
    )
...
def append_pre_order(
    ...
    *,
    ...
    if any(
        isinstance(entry, PreOrderJournalEntry) and entry.idempotency_key == idempotency_key
        for entry in entries
    ):
        raise SideEffectJournalError("journal idempotency key already has a pre-order entry")
    try:
...
def append_post_order(
    ...
    *,
    ...
    entries = read_journal(path)
    planned_entries = {
        entry.entry_hash: entry for entry in entries if isinstance(entry, PreOrderJournalEntry)
    }
    ...
    if stored_planned.execution_mode != execution_mode:
        raise SideEffectJournalError("post-order execution mode does not match pre-order entry")
    if any(
        isinstance(entry, PostOrderJournalEntry)
        and entry.idempotency_key == planned.idempotency_key
        for entry in entries
    ):
        raise SideEffectJournalError("journal idempotency key already has a post-order entry")
    ...
    *,
    ...
    planned = next(
        (
            entry
            for entry in entries
            if isinstance(entry, PreOrderJournalEntry) and entry.idempotency_key == idempotency_key
        ),
        None,
    )
    if planned is None:
        raise SideEffectJournalError("journal has no pre-order entry for idempotency key")
    result = next(
        (
            entry
            for entry in entries
            if isinstance(entry, PostOrderJournalEntry) and entry.idempotency_key == idempotency_key
        ),
        None,
    )
    if result is None:
        raise SideEffectJournalError("journal has no post-order result for idempotency key")
    if require_real and (planned.execution_mode != "real" or result.execution_mode != "real"):
        raise SideEffectJournalError("journal order completion is not a real execution")
