...
def _bootstrap_revision(
    ...
    if revision is None:
        return (
            None,
            used_record,
            ("bootstrap revision unavailable; the committed source changes cannot be enumerated"),
        )
...
def collect_source_changes(
    ...
    *,
    ...
    revision, used_record, failure = _bootstrap_revision(root, bootstrap_revision, bootstrap_record)
    ...
    if ancestor.returncode != 0:
        return section.model_copy(
            update={"reason": (f"bootstrap revision {revision} is not an ancestor of HEAD")}
        )
    commits_result = _git(root, "log", "--format=%H", f"{revision}..HEAD")
    log_result = _git(root, "log", "--stat", "--format=%H %s", f"{revision}..HEAD")
    if commits_result.returncode != 0 or log_result.returncode != 0:
        return section.model_copy(
            update={
                "reason": (
                    f"git log failed: {(commits_result.stderr or log_result.stderr).strip()}"
                )
            }
        )
...
def render_final_report_basis(report: FinalReportBasis) -> str:
    ...
    lines.append("")
    lines.append("worktree entries (path | status | modified_at | change actions)")
    lines.append("| path | status | modified_at | change actions |")
    lines.append("|---|---|---|---|")
    for entry in section.worktree_entries:
        actions = (
            "; ".join(
                f"{action.sequence}@{action.recorded_at} {action.tool_name}:{action.action}"
                for action in entry.change_actions
            )
            or "none recorded"
        )
        lines.append(
            f"| {entry.path} | {entry.status_code} | {entry.modified_at or 'unknown'} | {actions} |"
        )
    ...
    if report.design_values is not None:
        ...
        for component in values.components:
            pads = ", ".join(
                f"{pad}->{net or 'no_connect'}" for pad, net in sorted(component.pads.items())
            )
            lines.append(
