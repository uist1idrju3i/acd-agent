        ...
        with self._lock:
            if self._started:
                raise RuntimeObservationError(
                    "timing record has unfinished stages: " + ", ".join(sorted(self._started))
                )
...
class StageArtifactCache:
    ...
    def key(stage: str, inputs: object) -> str:
        ...
        return canonical_json_sha256({"stage": stage, "inputs": inputs}).removeprefix("sha256:")
    ...
    def get(self, stage: str, key: str, suffix: str) -> bytes | None:
        ...
        try:
            ...
            if (
                ...
                or record.get("content_sha256") != "sha256:" + hashlib.sha256(data).hexdigest()
            ):
