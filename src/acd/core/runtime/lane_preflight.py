...
from acd.core.manufacturing.part_selection import PartSelectionError, load_parts_catalog
from acd.core.mechanical.mechanical_preflight import collect_mechanical_findings
...
def _lane_report(
    ...
    return LanePreflightLaneReport(
        ...
        warnings=warnings,
        firmware_coverage=(
            _firmware_coverage_diagnostic(graph) if lane == "firmware-pipeline" else None
        ),
    )
    ...
    for (catalog_id, catalog_sha), nodes in groups.items():
        ...
        if reason is None:
            if actual == catalog_sha:
                continue
            refdes_list = [str(node.attrs.get("refdes") or node.id) for node in nodes]
            reason = (
...
def run_lane_preflight(
    ...
    *,
    ...
    reports = [
        _lane_report(graph, lane, LANE_REQUIREMENTS[lane], root, evidence_root) for lane in selected
    ]
    status = (
        "declarations_complete"
    ...
    return LanePreflightReport(
        ...
        lanes=reports,
        producer_gaps=collect_producer_gaps(graph, root=root, evidence_root=evidence_root),
    )
        ...
        for kind in kinds:
            ...
            entries.append(
                LanePreflightMissingDeclaration(
                    ...
                    required_attrs=(list(requirement.attrs) if requirement is not None else []),
...
def missing_declaration_action(report: LanePreflightReport) -> str | None:
    ...
    if unsupported_parts:
        action += " Unsupported declared values: " + "; ".join(unsupported_parts)
    if report.producer_gaps:
        action += " Producer gaps: " + "; ".join(
            gap.producer + " (" + gap.detail + ")" for gap in report.producer_gaps
        )
    return action
