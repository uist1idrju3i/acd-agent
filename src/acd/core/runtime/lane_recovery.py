...
class LaneRecoveryDeclarations:
    ...
    def lane(self, lane_id: str) -> LaneRecoveryDeclaration | None:
        return next((item for item in self.document.lanes if item.lane_id == lane_id), None)
...
def load_lane_recovery_declarations(
    ...
    declaration_path = path or repository_root() / "contracts" / "lane-recovery-declaration.json"
    try:
...
def resolve_lane_recovery(
    ...
    *,
    ...
    enabled = {item.dimension_id: item.search_enabled for item in freedom_declaration.dimensions}
    disabled = sorted(
        dimension
