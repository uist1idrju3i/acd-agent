...
from acd.core.runtime.lane_preflight import run_lane_preflight
...
def _subject(predicate: dict[str, Any], remediation: dict[str, Any]) -> dict[str, Any]:
    subject: dict[str, Any] = {
        ...
        "change_dimensions": sorted(cast(list[str], remediation.get("change_dimensions", []))),
    }
...
def _fixture_rationale_coverage(fixture_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        document = RationaleDocument.model_validate_json(rationale_path.read_text(encoding="utf-8"))
    ...
    return {
        ...
        "unclassified": [item.model_dump(mode="json") for item in report.unclassified],
    }
...
def _fixture_lane_preflight(fixture_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise GateDiagnosisError(f"fixture graph cannot be read: {graph_path}: {exc}") from exc
        ...
        if isinstance(observation, dict):
                ...
                for predicate in predicates:
                    ...
                    if predicate.get("status") in {"fail", "unknown"}:
                        ...
                        if isinstance(remediation, dict):
                            ...
                            raw_dimensions: object = remediation.get("change_dimensions", [])
            ...
            for key in ("unconnected_nets", "unconnected_net_ids"):
                ...
                if values is not None:
                    ...
                    unconnected_nets.update(cast(list[str], net_values))
