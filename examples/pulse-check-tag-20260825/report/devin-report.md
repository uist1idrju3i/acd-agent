...
2. **基板lane以降は通過していない。** digest固定container内のFreeRoutingが、pass予算
...
3. **authoritative Evidenceは1件も成立していない。** silkscreen laneはdigest固定container内で
...
4. **fail-closedは全段で正しく働いた。** 属性不足、`unknown`、library不足、timeoutのいずれも
   合格側へ倒れず、11件の設計不足を順に検出した（§5）。本検証で閾値・期待値・evidence規則を
   緩めた箇所は無い。
5. **実機側の実行基盤が今回の律速である。** container gate実行中にOpenHands側で
...
| FW | digest固定container | **未実行**（実機OpenHandsの停止により到達せず） | 未検証 |
...
- そのたびにホストのsshdが数分〜数十分不応答になり（ICMPは応答）、最終的にOpenHands
  サーバプロセス自体が停止した。
- `src/acd/openhands/container_runtime.py`の既定は`DEFAULT_MEMORY_LIMIT = "8g"`で、
  `--memory=8g --memory-swap=8g`を付けて`docker run`する。ホスト実装容量との整合は
  検査していない（Q-2）。
...
9件を[`improvement-notes.md`](improvement-notes.md)へQ-1〜Q-9として整理した。優先度上位は次の3件。
...
3. lane必須宣言の一括preflightを、各laneの入口検査と同一述語で提供する（Q-4、Q-5）。
